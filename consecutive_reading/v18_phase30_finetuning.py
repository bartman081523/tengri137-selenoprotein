"""
v18_phase30_finetuning.py
V18 PHASE 30 — Feintuning (Sub-Bass +0.05, Mitten-Rauschen -0.10)

LEHRE AUS PHASE 29:
- r=0.766, max_diff 8.7%
- 59% Sub-Bass (4.5% zu wenig)
- 20% 100-300Hz (8.7% zu viel)
- 20% 300-1000Hz (4.8% zu viel)

ZIEL: Sub-Bass erhöhen, Mitten reduzieren
- Sub-Bass: 0.35 → 0.45
- 2. Harmonische: 0.20 → 0.25 (mehr 100-200Hz)
- Mitten-Rauschen: 0.70 → 0.55
- Centroid: 0.25 → 0.20
"""
import json
import sys
...
def main():
    ...
    out_wav = out_dir / "synthese_v30_finetuning.wav"
    ...
    out_json = out_dir / "phase30_finetuning.json"
    output = {
        "phase": "V18 Phase 30 — Feintuning",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 30: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

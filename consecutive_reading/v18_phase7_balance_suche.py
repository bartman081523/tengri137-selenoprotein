"""
v18_phase7_balance_suche.py
V18 PHASE 7 — FINESSE-BALANCE: Original-Höhenprofil

LEHRE AUS PHASE 1-6:
- v1: r=0.339 (zu hell, Centroid 1.36)
- v2: r=0.335 (zu bass, Centroid 0.65)
- v3: r=0.351 (Centroid 0.74, Sub-Bass fehlt, Low-Mid zu viel)
- v4: r=0.255 (iterative Anpassung zerstörte es)
- v5: r=0.386 (BESTER, 100-300Hz +18% LECK)
- v6: r=0.361 (Sub-Bass 74% ZU VIEL)

MUSTER:
- Band-Match im 100-300Hz ist schwer
- 0-100Hz schwankt zwischen 50% und 75%
- r schwankt zwischen 0.25 und 0.39

NEUE STRATEGIE: VEREINFACHUNG
- Original-Profil: 64% Sub / 12% Bass / 16% Low-Mid / 6% Mid / 4% High
- Ziel: 5 Sinus-Töne mit GENAU den richtigen Amplituden
- Bestimme Amplituden DURCH numerische Optimierung (grid search)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 7 — FINESSE-BALANCE (numerische Optimierung)")
    print("=" * 80)
    ...
    # === Kostenfunktion: 5 Sinus-Töne finden, die + BURUMUT das Original nachbilden ===
    # Sub-Bass: 50Hz (drone)
    # Bass: 200Hz
    # Low-Mid: 600Hz
    # Mid: 1500Hz
    ...
    def cost(amps):
        ...
        # Korrelation maximieren = -1 * r
        r = float(np.corrcoef(log_s, log_o)[0, 1])
        return -r  # minimize

    # Initiale Amplituden
    amps0 = np.array([0.4, 0.08, 0.10, 0.04, 0.005])
    print(f"\nInitiale Amplituden: {amps0}")

    # Schneller Grid-Search
    best_r = -cost(amps0)
    best_amps = amps0.copy()
    print(f"Initial r = {best_r:.3f}")

    # Optimierung
    result = minimize(cost, amps0, method='Nelder-Mead',
                       options={'maxiter': 50, 'xatol': 0.001, 'fatol': 0.001})
    amps_opt = result.x
    r_opt = -result.fun
    print(f"Optimiert r = {r_opt:.3f}")
    print(f"Amplituden: {dict(zip(freqs_target, amps_opt))}")

    # Baue finale Synthese
    out = burumut.copy()
    ...
    out_wav = out_dir / "synthese_v7.wav"
    ...
    print("VERGLEICH")
    print("-" * 80)
    print(f"  r={r_opt:.3f}, ratio={ratio:.3f}")
    ...
    tests.append({"name": "T2_bands", "pass": bool(max_diff < 0.05),
                  ...
                  "was_sagt_es_uns": f"Band-Match (Ziel <5%): {'OK' if max_diff < 0.05 else 'ABWEICHUNG'}."})
    tests.append({"name": "T3_r", "pass": bool(r_opt > 0.6),
                  "befund": f"r={r_opt:.3f}",
                  "was_sagt_es_uns": f"Spektrum-Form (Ziel >0.6): {'OK' if r_opt > 0.6 else 'SCHWACH'}."})
    ...
    out_json = out_dir / "phase7_balance_suche.json"
    output = {
        "phase": "V18 Phase 7 — Finesse-Balance",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r_opt),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 7: {n_pass}/{len(tests)} PASS. r={r_opt:.3f}, max_diff={max_diff*100:.1f}%, ratio={ratio:.3f}.",
    }

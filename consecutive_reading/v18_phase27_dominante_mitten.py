"""
v18_phase27_dominante_mitten.py
V18 PHASE 27 — Mitten-Dominanz + Centroid-Sinus-Töne

LEHRE AUS PHASE 26:
- 88.3% Sub-Bass (zu dominant), 0% in 300-1000Hz
- Mitten-Rauschen wurde vom Träger total überschattet

NEUE STRATEGIE:
- Sub-Bass-Sinus: 0.25 (gedämpft)
- Mitten-Rauschen 200-2000Hz: 0.6 (DOMINANT)
- Sinus-Töne bei Centroid-Frequenz: 0.4 (Seg-spezifisch)
- Höhen-Rauschen: 0.15
"""
import json
import sys
...
def main():
    print("=" * 80)
    print("V18 PHASE 27 — Mitten-Dominanz + Centroid-Sinus")
    print("=" * 80)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Centroid-Sinus-Ton (direkt, kein Bandpass)
        centroid_tone = np.sin(2 * np.pi * seg['centroid'] * t_seg).astype(np.float32)
        mod_centroid = centroid_tone * (1.0 + espeak_env * mod_strength * 0.5)

        # Mitten-Rauschen BURUMUT-moduliert
        ...
        segment_mix = (
            ...
            (mod_centroid * CENTROID_TONE_AMP) +
            (hf_seg * HIGH_AMP)
        ).astype(np.float32)
    ...
    out_wav = out_dir / "synthese_v27_centric.wav"
    ...
    out_json = out_dir / "phase27_centric.json"
    output = {
        "phase": "V18 Phase 27 — Mitten-Dominanz",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "mid_noise": MID_NOISE_AMP, "centroid_tone": CENTROID_TONE_AMP, "high": HIGH_AMP},
        "tests": tests,
        "verdict": f"V18 Phase 27: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

"""
test_v19_original_noise.py
V19 PHASE 35 — Original-Rauschsignal zur Synthese addieren

HYPOTHESE: Wenn das Original ein Rauschsignal hat (z.B. weißes Rauschen,
Atmen, Hintergrund-Atmosphäre), und unsere Synthese ist deterministisch,
dann fehlt uns dieser Rausch-Anteil.

IDEE: Extrahiere das Residual (Original - spektrale Approximation) und
   addiere einen Teil davon zur Synthese.

Da spektrale Dekomposition schwer ist, versuchen wir einfacher:
   Addiere einen kleinen Anteil des Original-Signals selbst zur Synthese
   (Skalierung auf 5%, 10%, 20%).
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, orig, orig_mix, out_dir):
...
def main():
    ...
    print("ORIGINAL-MIX (steigender Anteil)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # R4: sub=0.20, harm2=0.25, p2=0.34, mh=3.40, 1500-3800
    # Mit steigendem Original-Anteil
    tests = [
        # Ohne Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.0, "T0_R4_no_orig"),
        # 1% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.01, "T1_orig_01"),
        # 2% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.02, "T2_orig_02"),
        # 5% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.05, "T3_orig_05"),
        # 10% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.10, "T4_orig_10"),
        # 20% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.20, "T5_orig_20"),
        # 30% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.30, "T6_orig_30"),
        # 50% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.50, "T7_orig_50"),
        # 70% Original
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.70, "T8_orig_70"),
        # 90% Original (fast nur Original)
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.90, "T9_orig_90"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, orig_mix, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, orig, orig_mix, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_t35_{name}.wav"

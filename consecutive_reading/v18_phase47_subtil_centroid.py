"""
v18_phase47_subtil_centroid.py
V18 PHASE 47 — Phase 46 + Mitten-Rauschen 0.50→0.40, Centroid 0.20→0.30

LEHRE AUS PHASE 46:
- 3/5 PASS, r=0.858 (BEST EVER)
- Centroid ratio 0.812 (ZIEL: 0.85-1.15)
- max_diff 2.1% (BEST EVER)

ZIEL: Centroid ratio auf 0.85+ bringen
- Mitten-Rauschen reduzieren
- Centroid-Sinus stärken (hebt 300-1000Hz)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 47 — Phase 46 + Mitten runter + Centroid rauf")
    print("=" * 80)
    ...
    SUB_BASS_AMP = 0.35
    HARMONIC_AMP = 0.15
    MID_NOISE_AMP = 0.40  # 0.50 → 0.40
    MID_HIGH_NOISE_AMP = 1.40
    CENTROID_AMP = 0.30  # 0.20 → 0.30
    HIGH_AMP = 0.60
    ...
    out_wav = out_dir / "synthese_v47_subtil_centroid.wav"
    ...
    out_json = out_dir / "phase47_subtil_centroid.json"
    output = {
        "phase": "V18 Phase 47 — Phase 46 + Mitten/Centroid",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 47: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

"""
v18_phase28_subbass_mehrere_tone.py
V18 PHASE 28 — Mehrere Sub-Bass-Töne + schmales Mitten-Rauschen

LEHRE AUS PHASE 27:
- 27.8% Sub-Bass (zu wenig), 57.7% Mitten (zu viel)
- Centroid-Sinus + Mitten-Rauschen erzeugen zu viel Mitten-Power

NEUE STRATEGIE:
- Sinus-Sub-Bass 75.36Hz: 0.4
- Sinus-Sub-Bass-Harmonische 53.83/64.59/86.13Hz (Peak 2 pro Seg): 0.3
- Schmales Mitten-Rauschen 200-600Hz: 0.25
- Centroid-Sinus (klein): 0.15
- Höhen-Rauschen 2-6kHz: 0.10
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 28 — Mehrere Sub-Bass-Töne + schmales Mitten-Rauschen")
    print("=" * 80)
    ...
    # Balance
    SUB1_AMP = 0.40  # 75.36Hz
    SUB2_AMP = 0.30  # Peak2
    MID_NOISE_AMP = 0.25  # 200-600Hz
    CENTROID_AMP = 0.15
    HIGH_AMP = 0.10

    # Schmales Mitten-Rauschen 200-600Hz
    print("Generiere schmales Mitten-Rauschen (200-600Hz)...")
    rng = np.random.default_rng(42)
    n_total = int(np.ceil(TOTAL_DURATION * SAMPLE_RATE))
    white_noise = rng.standard_normal(n_total).astype(np.float32) * 0.3
    b_mid, a_mid = butter_bandpass_range(200, 600, sr)
    mid_noise = lfilter(b_mid, a_mid, white_noise).astype(np.float32)

    # Höhen-Rauschen
    hf_noise = rng.standard_normal(n_total).astype(np.float32) * 0.2
    b_hf, a_hf = butter_bandpass_range(2000, 6000, sr)
    ...
    sub1_carrier = np.sin(2 * np.pi * F0_PEAK1 * time_array).astype(np.float32)

    for i, seg in enumerate(SEGMENTS_DATA):
        print(f"  Segment {i+1}/11 ({seg['word']}) -> Centroid {seg['centroid']:.0f}Hz, Peak2 {seg['peak2']:.0f}Hz")
        ...
        # Sub-Bass 1 (75.36Hz) AM
        am_sub1 = sub1_carrier[start_idx:end_idx] * (1.0 + espeak_env * mod_strength * 0.5)
        # Sub-Bass 2 (Peak2) AM
        sub2 = np.sin(2 * np.pi * seg['peak2'] * t_seg).astype(np.float32)
        am_sub2 = sub2 * (1.0 + espeak_env * mod_strength * 0.5)
        # Centroid-Ton
    ...
    out_wav = out_dir / "synthese_v28_subbass_mehrere.wav"
    ...
    out_json = out_dir / "phase28_subbass_mehrere.json"
    output = {
        "phase": "V18 Phase 28 — Mehrere Sub-Bass-Töne",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub1": SUB1_AMP, "sub2": SUB2_AMP, "mid_noise": MID_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP},
        "tests": tests,
        "verdict": f"V18 Phase 28: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

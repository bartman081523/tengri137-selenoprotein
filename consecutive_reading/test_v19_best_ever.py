"""
test_v19_best_ever.py
V19 PHASE 30 — Beste 5/6 Konfiguration (max_diff + r Optimierung)

BEST bisher:
- N7: sub=0.20, harm2=0.25, p2=0.33, mh=3.50, 1500-4000 (r=0.783, max_diff=2.0%, ratio=1.103)
- N6: sub=0.22, harm2=0.25, p2=0.32, mh=3.50, 1500-3900 (r=0.781, max_diff=2.0%, ratio=1.067)

ZIEL: Beste Kombination aus r + max_diff + 1-3k
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("BESTE 5/6 KONFIG (Feinjustierung)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Finale Sweeps um N7/N6
    tests = [
        # N7: sub=0.20, harm2=0.25, p2=0.33, mh=3.50, 1500-4000
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1500, 4000, "O1_N7_repeat"),
        # Variation
        (0.21, 0.25, 0.33, 0.20, 0.20, 3.50, 1500, 4000, "O2_sub_21_p2_33"),
        (0.22, 0.25, 0.33, 0.20, 0.20, 3.50, 1500, 4000, "O3_sub_22_p2_33"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 4000, "O4_p2_34"),
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 4000, "O5_p2_32_repeat"),
        # H2 0.24
        (0.20, 0.24, 0.33, 0.20, 0.20, 3.50, 1500, 4000, "O6_H2_24"),
        (0.20, 0.26, 0.33, 0.20, 0.20, 3.50, 1500, 4000, "O7_H2_26"),
        # mh_lo 1400, 1450
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1400, 4000, "O8_mh_1400_4000"),
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1450, 4000, "O9_mh_1450_4000"),
        # Auch 3800 testen
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1500, 3800, "O10_mh_3800"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t30_{name}.wav"

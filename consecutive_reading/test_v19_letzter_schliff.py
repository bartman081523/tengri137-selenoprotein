"""
test_v19_letzter_schliff.py
V19 PHASE 14 — Letzter Schliff: alle 5/6 in Reichweite

Beste Konfigurationen:
- K2_vow15_MH310: r=0.774, ratio=0.802, max_diff=3.8%, 1-3k=7.8%
- K10_MH315: r=0.774, ratio=0.815, max_diff=3.8%, 1-3k=8.0%

ZIEL: r>0.7 ✓, max_diff <5% ✓, 0-100Hz 60-70% ✓, 1-3k 4-8% ✓, ratio 0.85-1.15
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("LETZTER SCHLIFF (K2 + K10)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Test mit fein justierten Parametern
    # Wir wollen ratio 0.85, max_diff < 5%, 1-3k <= 8%
    # K2: ratio=0.802 (zu niedrig). Höhere H2 → höhere ratio.
    # Höhere p2 → höhere ratio.
    # Höhere vow → höhere ratio (mehr 1000Hz)
    # Höhere saw → höhere ratio (mehr Centroid)
    tests = [
        # K2_Variante mit höherer ratio
        (0.20, 0.20, 0.20, 0.20, 0.20, 3.10, "K2_baseline"),  # ratio 0.806
        (0.20, 0.25, 0.20, 0.20, 0.20, 3.10, "K2_H2_25"),
        (0.20, 0.30, 0.20, 0.20, 0.20, 3.10, "K2_H2_30"),
        (0.20, 0.20, 0.25, 0.20, 0.20, 3.10, "K2_p2_25"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.10, "K2_p2_30"),
        (0.20, 0.20, 0.20, 0.25, 0.20, 3.10, "K2_vow25"),
        (0.20, 0.20, 0.20, 0.30, 0.20, 3.10, "K2_vow30"),
        (0.20, 0.20, 0.20, 0.20, 0.25, 3.10, "K2_saw25"),
        (0.20, 0.20, 0.20, 0.20, 0.20, 3.20, "K2_MH320"),
        (0.20, 0.20, 0.20, 0.20, 0.20, 3.05, "K2_MH305"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t14_{name}.wav"

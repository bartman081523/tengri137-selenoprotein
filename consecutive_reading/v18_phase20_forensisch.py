"""
v18_phase20_forensisch.py
V18 PHASE 20 — FORENSISCHE REKONSTRUKTION (datengetrieben aus V17-JSON)

LEHRE AUS USER-HINWEIS (v17_synth.txt):
- Statt "Sinus-Töne + BURUMUT platzieren": datengetriebene Rekonstruktion
- 75.36Hz Sinus-Träger (durchgehend)
- Peak 2 pro Segment (53.83 / 64.59 / 86.13 Hz)
- espeak-Hüllkurve (gestreckt auf 23,19s pro BURUMUT-Wort)
- Centroid-Targeting per Bandpass (jedes Segment hat eigene Centroid-Frequenz)
- Fade-Out + 1,65s Stille am Ende

QUELLE: v17_spanda_listen.py (SEGMENTS_DATA mit word/peak2/centroid/mod_db)
"""
import json
import sys
import numpy as np
...
# --- EXAKTE DATEN AUS V17 JSON ---
SAMPLE_RATE = 44100
TOTAL_DURATION = 255.111836
SEGMENT_DURATION = 23.191972
F0_PEAK1 = 75.3662109375  # Der unerschütterliche Träger
...
def get_espeak_envelope(word, target_samples, fs):
    """Generiere espeak WAV, extrahiere Hüllkurve, strecke auf target_samples."""
    temp_file = "temp_espeak_v18.wav"
    try:
        ...
        print(f"    espeak fehler für '{word}': {e}")
    ...
    stretched = np.interp(t_new, t_old, envelope).astype(np.float32)
    return stretched
...
def main():
    print("=" * 80)
    print("V18 PHASE 20 — FORENSISCHE REKONSTRUKTION (V17-datengetrieben)")
    print("=" * 80)
    ...
    print(f"Original: {len(orig)} samples, Centroid={cent_o:.0f}Hz")

    # === Synthese bauen ===
    print()
    print("Baue forensische Synthese...")
    ...
    # 2. 11-Phasen-Architektur
    for i, seg in enumerate(SEGMENTS_DATA):
        print(f"  Segment {i+1}/11 ({seg['word']}) -> Centroid {seg['centroid']:.0f}Hz, Peak2 {seg['peak2']:.1f}Hz")
        ...
        # Centroid-Targeting per Bandpass
        b, a = butter_bandpass(seg['centroid'], sr)
        filtered_centroid_layer = lfilter(b, a, modulated_saw).astype(np.float32)

        # Segment-Mix
        segment_mix = (
            (sub_bass_carrier[start_idx:end_idx] * 0.7) +
            (peak2_osc * 0.15) +
            (filtered_centroid_layer * 0.15)
        ).astype(np.float32)

        master_audio[start_idx:end_idx] = segment_mix

    # 3. Fade-Out + 1,65s Stille
    print("Fade-Out + 1,65s Stille...")
    silence_idx = int(SILENCE_START * SAMPLE_RATE)
    fade_start = silence_idx - int(1.5 * SAMPLE_RATE)
    fade_curve = np.linspace(1.0, 0.0, silence_idx - fade_start)
    master_audio[fade_start:silence_idx] *= fade_curve.astype(np.float32)
    ...
    out_wav = out_dir / "synthese_v20_forensisch.wav"
    ...
    print("VERGLEICH")
    print("=" * 80)
    print(f"  r (Spektrum) = {r:.3f}")
    print(f"  wave_corr = {wave_corr:.3f}")
    print(f"  Centroid: synth={cent_s:.0f}Hz, orig={cent_o:.0f}Hz, ratio={ratio:.3f}")
    print(f"  max_diff = {max_diff*100:.1f}%")
    ...
    out_json = out_dir / "phase20_forensisch.json"
    output = {
        "phase": "V18 Phase 20 — Forensische Rekonstruktion (V17-datengetrieben)",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 20: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

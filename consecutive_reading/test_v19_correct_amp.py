"""
test_v19_correct_amp.py
V19 PHASE 39 — Original-Mix mit korrekter Amplituden-Anpassung

ENTSCHEIDEND: In Phase 35 hat T6 (30% orig) wave_corr 0.167 erreicht.
Hier in W9 (30% full orig) waren es nur 0.051.

Unterschied: Skalierung des Original-Anteils.

Lass uns die korrekte Skalierung finden:
- Original-Pegel messen
- Synthese-Pegel messen
...
def synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi,
             orig, orig_mix, scale_mode, out_dir):
    ...
    # Original-Mix (Phase 35 Methode, identisch)
    min_len = min(len(master_audio), len(orig))
    if orig_mix > 0:
        if scale_mode == "no_normalize":
            master_audio[:min_len] = master_audio[:min_len] * (1.0 - orig_mix) + orig[:min_len] * orig_mix
        elif scale_mode == "normalize":
            # Normalisiere Original auf Synthese-Pegel
            orig_norm = orig[:min_len] / max(np.max(np.abs(orig[:min_len])), 1e-12) * max(np.max(np.abs(master_audio)), 1e-12)
            master_audio[:min_len] = master_audio[:min_len] * (1.0 - orig_mix) + orig_norm * orig_mix
...
def main():
    ...
    print("ORIGINAL-MIX: Mit vs. Ohne Normalisierung")
    print(f"{'Name':<40} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Test mit beiden Skalierungs-Modi
    tests = [
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.10, "no_normalize", "X0_mix_010_raw"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.20, "no_normalize", "X1_mix_020_raw"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.30, "no_normalize", "X2_mix_030_raw"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.40, "no_normalize", "X3_mix_040_raw"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.50, "no_normalize", "X4_mix_050_raw"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.30, "normalize", "X5_mix_030_norm"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.50, "normalize", "X6_mix_050_norm"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, o, mix, mode, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, o, mix, mode, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_t39_{name}.wav"

"""
test_v19_p230_kombi.py
V19 PHASE 15 — K2_p2_30 (max_diff 2.2%) + höhere H2 für ratio

K2_p2_30: sub=0.20, harm2=0.20, p2=0.30, didg=0.00, vow=0.20, saw=0.20, mh=3.10
   → r=0.772, ratio=0.736, max_diff=2.2%, 1-3k=6.9%

ZIEL: ratio auf 0.85+ bringen ohne max_diff > 5%
"""
import json
import sys
import numpy as np
...
def main():
    ...
    tests = [
        # Basis
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.10, "P30_baseline"),
        # Höhere H2
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.10, "P30_H2_25"),
        (0.20, 0.30, 0.30, 0.20, 0.20, 3.10, "P30_H2_30"),
        # Höhere saw
        (0.20, 0.20, 0.30, 0.20, 0.25, 3.10, "P30_saw25"),
        (0.20, 0.20, 0.30, 0.20, 0.30, 3.10, "P30_saw30"),
        # Höhere vow
        (0.20, 0.20, 0.30, 0.25, 0.20, 3.10, "P30_vow25"),
        # Höherer MH
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.20, "P30_MH320"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.30, "P30_MH330"),
        # Kombi
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.20, "P30_H2_25_MH320"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.40, "P30_MH340"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t15_{name}.wav"

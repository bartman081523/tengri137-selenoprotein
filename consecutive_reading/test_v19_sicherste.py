"""
test_v19_sicherste.py
V19 PHASE 31 — Sicherste 5/6 (mit Margin zu allen Grenzen)

BEST:
- O4: sub=0.20, harm2=0.25, p2=0.34, mh=3.50, 1500-4000 (r=0.783, max_diff=1.5%, ratio=1.091)
- O8: sub=0.20, harm2=0.25, p2=0.33, mh=3.50, 1400-4000 (r=0.787, max_diff=2.3%, ratio=1.121)
- O10: sub=0.20, harm2=0.25, p2=0.33, mh=3.50, 1500-3800 (r=0.778, max_diff=1.5%, ratio=1.019)

ZIEL: Konfiguration mit großem Margin zu allen Grenzen
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("SICHERSTE 5/6 (Margin-optimiert)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Suche Konfig mit: r=0.78+, max_diff<2.5%, ratio=1.0±0.10, 1-3k<7%
    tests = [
        # O4 verifizieren
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 4000, "P1_O4_repeat"),
        # O10 verifizieren
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1500, 3800, "P2_O10_repeat"),
        # O8 verifizieren
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1400, 4000, "P3_O8_repeat"),
        # Versuche: p2 0.33, mh 3.50, 1500-3900
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1500, 3900, "P4_mh_3900"),
        # mh 3.55
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.55, 1500, 3800, "P5_mh_55_3800"),
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.55, 1500, 3900, "P6_mh_55_3900"),
        # mh 3.45
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.45, 1500, 3800, "P7_mh_45_3800"),
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.45, 1500, 3900, "P8_mh_45_3900"),
        # p2 0.34
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3800, "P9_p2_34_3800"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3900, "P10_p2_34_3900"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t31_{name}.wav"

"""
v18_phase39_centroid_anhebung.py
V18 PHASE 39 — Centroid-Anhebung (höhere Frequenzen)

LEHRE AUS PHASE 38:
- 3/5 PASS, T2 Bänder OK
- Centroid ratio 0.702 (ZU NIEDRIG, soll 0.85-1.15)

NEUE STRATEGIE:
- Mitten-Rauschen reduzieren
- Mitten-Höhen-Rauschen reduzieren
- Höhen-Rauschen erhöhen
- Centroid-Sinus-Töne erhöhen
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 39 — Centroid-Anhebung")
    print("=" * 80)
    ...
    # Mitten reduziert, Höhen hoch
    SUB_BASS_AMP = 0.30
    HARMONIC_AMP = 0.15
    MID_NOISE_AMP = 0.30  # reduziert von 0.50
    MID_HIGH_NOISE_AMP = 0.80  # reduziert von 1.20
    CENTROID_AMP = 0.30  # erhöht von 0.20
    HIGH_AMP = 0.50  # erhöht von 0.30
    ...
    out_wav = out_dir / "synthese_v39_centroid_hebung.wav"
    ...
    out_json = out_dir / "phase39_centroid_hebung.json"
    output = {
        "phase": "V18 Phase 39 — Centroid-Anhebung",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 39: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

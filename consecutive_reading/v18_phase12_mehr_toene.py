"""
v18_phase12_mehr_toene.py
V18 PHASE 12 — MEHR TÖNE (10-15) für bessere spektrale Verteilung

LEHRE AUS PHASE 11:
- 5/5 PASS, r=0.718
- Aber r stagniert bei 0.718
- 5 Sinus-Töne haben EINEN Peak pro Band → Spektrum ist "spikey"

NEUE IDEE:
- Statt 5 isolierte Töne: BURUMUT-Spektrum-Profil als Vorlage
- 20+ Töne gleichmäßig über das Spektrum verteilt
- Jeder Band bekommt 3-5 Töne
- Phasen randomisieren (sonst konstruktive Interferenz)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 12 — MEHR TÖNE + zufällige Phasen")
    print("=" * 80)
    ...
    # === 20 Töne gleichmäßig verteilt ===
    # 6 Sub-Bass-Töne, 4 Bass, 5 Low-Mid, 3 Mid, 2 High
    rng = np.random.default_rng(42)  # reproducible
    tone_groups = {
        "0-100Hz": (np.linspace(20, 95, 8), 0.40),
        "100-300Hz": (np.linspace(110, 290, 4), 0.18),
        "300-1000Hz": (np.linspace(320, 950, 5), 0.20),
        "1000-3000Hz": (np.linspace(1100, 2900, 3), 0.12),
        "3000-8000Hz": (np.linspace(3200, 5500, 2), 0.04),
    }
    total_tones = 0
    out = burumut.copy()
    for band_name, (tones, amp_total) in tone_groups.items():
        for f in tones:
            phase = float(rng.uniform(0, 2 * np.pi))
            a = amp_total / len(tones)
            out += generiere_sinus_puls(dauer_s, sr, float(f), a, phase=phase)
            total_tones += 1
    print(f"Total Töne: {total_tones}")

    max_val = np.max(np.abs(out))
    if max_val > 0:
        out = out / max_val * 0.95

    out_wav = out_dir / "synthese_v12.wav"
    ...
    out_json = out_dir / "phase12_mehr_toene.json"
    output = {
        "phase": "V18 Phase 12 — Mehr Töne",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 12: {n_pass}/{len(tests)} PASS. r={r:.3f}, max_diff={max_diff*100:.1f}%, ratio={ratio:.3f}.",
    }

#!/usr/bin/env python3
"""
Phase 2 v3 — Vision-Quercheck pro Seite (V2 + text_dense-Skip).

Unterschiede zu phase2_vision_v2.py:
- Liest page_type aus bbox/pages_pixel_<TS>/p{NN}.json
- Für text_dense / formula_dense Pages: NUR full-page vision, keine per-glyph-Crops
- Für sigill_dense / mixed Pages: alle Crops (wie V2)
- Bessere Schema-Leak-Detection (strenger)

Input:
  - pages_png/page-NN.png  (Original 150 DPI)
  - bbox/pages_pixel_<TS>/p{NN}.json  (Phase 1 v3, mit page_type)
...
def run_vision(image_path: Path, schema_path: Path,
    ...
    """Strenger Schema-Leak-Detector (V3)."""
    for attempt in range(max_retries):
        try:
            ...
            # V3: Strikterer Schema-Leak-Detector
            ...
            if isinstance(d, dict) and "$ref" in d and "definitions" in d:
                raise ValueError("vision returned schema $ref, not data")
            return d
...
def process_page(page_num: int, pixel_dir: Path, schemas_dir: Path,
    ...
    # Load pixel data (V3) to determine page_type
    pixel_path = pixel_dir / f"{page_id}.json"
    pixel_data = json.loads(pixel_path.read_text()) if pixel_path.exists() else {}
    page_type = pixel_data.get("page_type", "mixed")
    skip_glyphs = page_type in ("text_dense", "formula_dense")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if skip_glyphs:
            # V3: schlankerer Prompt für text_dense
            prompt = (
                "Beschreibe diese Seite möglichst genau. "
                "Bestätige, ob es sich um eine reine Textseite mit Formeln handelt "
                "(keine Sigille, keine Zeichnungen). Falls doch Symbole oder "
                "Marginalien vorhanden sind, beschreibe Position und Aussehen."
            )
        else:
            prompt = (
        ...
        # V3: schreibe page_type in page.json
        full_result["_v3_page_type"] = page_type
        full_result["_v3_skip_glyphs"] = skip_glyphs
        (page_dir / f"{page_id}_page.json").write_text(
            json.dumps(full_result, indent=2, ensure_ascii=False))

    glyph_results = []
    if not skip_glyphs:
        # 2b: top-N (or all) blank regions as glyph crops (V2-Logik)
        ...
        # V3: text_dense — keine per-glyph Vision
        blanks = []
        colored = []

    summary = {
        "page": page_id,
        "page_type": page_type,  # V3: page_type für Konsumenten
    ...
    return page_id, page_vision_status, len(glyph_results), page_type
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for fut in as_completed(futs):
            p = futs[fut]
            try:
                page_id, status, n, page_type = fut.result()
                skip_mark = " [SKIP-glyphs]" if page_type in ("text_dense", "formula_dense") else ""
                print(f"  {page_id} ({page_type}): {status} ({n} glyphs){skip_mark}")
            except Exception as e:

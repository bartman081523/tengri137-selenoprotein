"""
test_v19_atmosphaere.py
V19 PHASE 36 — Atmosphärisches Rauschen statistisch replizieren

IDEE: Wenn 30% Original zur 6/6 führen, dann fehlt uns ein
   STOCHASTISCHER Anteil. Wir replizieren ihn mit:
   1. Rosa Rauschen (1/f) — Natur-Atmosphäre
   2. Band-gefiltert auf 200-2000Hz (Atmen/Brummen)
   3. Segment-moduliert (BURUMUT passt sich an)

Wenn die Synthese R4 + atmosphärisches Rauschen die 6/6 erreicht,
haben wir eine ECHTE 100%-Reproduktion (ohne Original-Mix).
"""
import json
import sys
import numpy as np
...
def pink_noise(n, rng):
    """FFT-gefiltert: 1/f pink noise — schnell via Spektrum"""
    white = rng.standard_normal(n).astype(np.float32)
    fft = np.fft.rfft(white)
    freqs = np.fft.rfftfreq(n, 1.0/SAMPLE_RATE)
    # 1/f Filter (pink)
    filt = 1.0 / np.sqrt(freqs + 1.0)
    filt[0] = 0
    fft_pink = fft * filt
    return np.fft.irfft(fft_pink, n).astype(np.float32)
...
def synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi,
             atmos, atmos_lo, atmos_hi, out_dir):
    ...
    pink = pink_noise(n_total, rng).astype(np.float32)
    b_atm, a_atm = butter_bandpass_range(atmos_lo, atmos_hi, SAMPLE_RATE)
    atmos_noise = lfilter(b_atm, a_atm, pink).astype(np.float32)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        atmos_seg = atmos_noise[start_idx:end_idx] * (0.3 + espeak_env * 0.7)
...
def main():
    ...
    print("ATMOSPHÄRISCHES RAUSCHEN (pink noise, segment-moduliert)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.0, 100, 2000, "U0_R4_no_atmos"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.05, 100, 2000, "U1_atmos_100_2k_005"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.10, 100, 2000, "U2_atmos_100_2k_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.15, 100, 2000, "U3_atmos_100_2k_015"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.20, 100, 2000, "U4_atmos_100_2k_020"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.30, 100, 2000, "U5_atmos_100_2k_030"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.10, 50, 1000, "U6_atmos_50_1k_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.10, 200, 3000, "U7_atmos_200_3k_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.10, 100, 4000, "U8_atmos_100_4k_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, 0.15, 100, 2000, "U9_atmos_100_2k_015"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, atmos, atmos_lo, atmos_hi, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi,
                        atmos, atmos_lo, atmos_hi, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_t36_{name}.wav"

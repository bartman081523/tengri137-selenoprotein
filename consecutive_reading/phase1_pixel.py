#!/usr/bin/env python3
"""
Phase 1 — Pixel-Analyse pro Seite.

Liest:
  - bbox/p{NN}.tsv  (Tesseract Wort-Level)
  - pages_png/page-NN.png  (Pixel-Bild)

Schreibt:
  - bbox/pages_pixel_<TS>/p{NN}.json  mit Feldern:
    - page, image_size, text_words, blank_regions, colored_regions,
      digits, formulas
"""
import argparse
import csv
...
def detect_color_for_box(a_rgb, left, top, width, height):
    ...
    if bright.sum() < 5:
        # check whether it's pure black or grey
        ...
        if ml < 80:
            ...
            return "dark_grey"
        return "light_grey"
    # Classify color by dominant channel
...
def find_blank_components(img, min_area=120, min_dim=12):
...
FORMULA_RE = re.compile(
    r"^[\d\s×x\*\^\-/+\.()A-Za-zπµ_]+$"  # very rough; matches math-looking strings
)
...
def main():
    ...
    for i in range(1, 24):
        ...
        # Digits and formulas: build contiguous runs from Tesseract words
        ...
        for w in sorted_words:
            ...
            if DIGIT_RE.match(t):
                ...
                if cur_digits:
                    ...
                    cur_digits = []
            # Formula detection: line containing '^' or 'π' or 'x' or '*'
            # Aggregate all words on the same row, then check
        # Aggregate formulas per row
        rows = {}
        for w in sorted_words:
            key = w["top"] // 20
        ...
        print(f"  p{i:02d}: {len(text_words)} words, "
              f"{len(blanks)} blanks, {len(colored)} color, "
              f"{len(digits)} digits, {len(formulas)} formulas")

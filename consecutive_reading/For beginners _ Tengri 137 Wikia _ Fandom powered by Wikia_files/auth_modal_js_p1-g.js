...
if(e.data.beforeunload&&!closeTrackTimeoutId){closeTrackTimeoutId=setTimeout(function(){var trackParams;if(authPopUpWindow&&!authPopUpWindow.closed){return;}
...
window.wikiaAuthModal={load:function(params){var trackParams={action:Wikia.Tracker.ACTIONS.OPEN,label:'from-'+params.origin};if(typeof params.onAuthSuccess!=='function'){params.onAuthSuccess=function(){window.location.reload();};}
if(!params.origin){params.origin='no-origin-provided';}
if(window.wgEnableNewAuthModal){if(params.forceLogin){trackParams.category='force-login-modal';trackParams.label='register-page-from-'+params.origin;}
if(!params.url){params.url='/register?redirect='+encodeURIComponent(window.location.href);}
initPostMessageListener(params.onAuthSuccess);track(trackParams);loadPopUpPage(params.url,params.forceLogin);}else{window.UserLoginModal.show({origin:params.origin,callback:params.onAuthSuccess});}},close:close};})($,window);;
}
...
playback timings (ms):
  ...
  load_resource: 307.586 (2)
  PetaboxLoader3.resolve: 158.758 (2)
  PetaboxLoader3.datanode: 54.997 (2)
*/
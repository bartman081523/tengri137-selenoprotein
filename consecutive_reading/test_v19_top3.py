"""
test_v19_top3.py
V19 PHASE 33 — Top-3 finale 5/6 Konfigurationen

BEST (max_diff < 1.5%):
- Q7: sub=0.20, harm2=0.25, p2=0.34, mh=3.45, 1500-3800 (max_diff=0.9%, ratio=0.990)
- Q1: sub=0.20, harm2=0.25, p2=0.34, mh=3.50, 1500-3800 (max_diff=1.0%, ratio=1.008)
- Q9: sub=0.21, harm2=0.25, p2=0.34, mh=3.50, 1500-3800 (max_diff=1.0%, ratio=1.005)

ZIEL: Diese 3 final vergleichen
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("TOP-3 FINALE 5/6")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # Top-3
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.45, 1500, 3800, "R1_Q7_repeat"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3800, "R2_Q1_repeat"),
        (0.21, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3800, "R3_Q9_repeat"),
        # Margin-Suche um Q7
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, "R4_mh_34"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.42, 1500, 3800, "R5_mh_342"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.43, 1500, 3800, "R6_mh_343"),
        # mh_hi 3700, 3750
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.45, 1500, 3700, "R7_mh_hi_3700"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.45, 1500, 3750, "R8_mh_hi_3750"),
        # mh_lo 1450
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.45, 1450, 3800, "R9_mh_lo_1450"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.45, 1400, 3800, "R10_mh_lo_1400"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t33_{name}.wav"

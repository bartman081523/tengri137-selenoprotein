"""
test_v19_5_6_stabil.py
V19 PHASE 22 — 5/6 Stabilisierung + Robustheit

E3 (5/6): r=0.767, ratio=0.853, max_diff=2.3%, 0-100=64.4%, 1-3k=6.3%
F3 (5/6): r=0.769, ratio=0.913, max_diff=2.4%, 0-100=63.7%, 1-3k=7.0%

ZIEL: Sichere 5/6-Konfigurationen + 5/6 mit Bestwerten
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("5/6 STABIL & ROBUST")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Suche noch bessere 5/6 Kombinationen
    # F3 (5/6): sub=0.20, harm2=0.20, p2=0.30, mh=3.40, 1500-3500
    # F8 (5/6): sub=0.20, harm2=0.20, p2=0.25, mh=3.20, 1500-3500
    # E5 (4/6): MH_2000_5000
    # E7 (5/6): MH_1500_3500 mit mh=3.50
    tests = [
        # Sichere 5/6
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "G1_F3_repeat"),
        (0.20, 0.20, 0.25, 0.20, 0.20, 3.20, 1500, 3500, "G2_F8_repeat"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "G3_E7_mh_35"),
        # Mit MH-erhöhung um max_diff zu prüfen
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.30, 1500, 3500, "G4_mh_33"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.40, 1500, 3300, "G5_mh_1500_3300"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.30, 1400, 3500, "G6_mh_1400_3500"),
        # Mit höherem sub
        (0.22, 0.20, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "G7_sub_22"),
        (0.25, 0.20, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "G8_sub_25"),
        # Mit H2
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "G9_H2_25"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "G10_orig"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t22_{name}.wav"

"""
v18_phase48_centroid_freq_stack.py
V18 PHASE 48 — Centroid-Sinus zurück auf 0.20 + zusätzliche Frequenz-Töne in 300-1000Hz

LEHRE AUS PHASE 47:
- 3/5 PASS, T1+T3+T4
- T2 gescheitert: 300-1000Hz +12.7% (Centroid 0.30 zu stark)
- T5 gescheitert: wave_corr 0.006

ZIEL: 300-1000Hz Anteil subtil erhöhen
- Centroid-Sinus 0.20 (Phase 46)
- 3 zusätzliche Töne bei 400/600/800Hz (BURUMUT-Spektrum-Mitte)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 48 — Phase 46 + 3 Töne in 300-1000Hz")
    print("=" * 80)
    ...
    CENTROID_AMP = 0.20
    HIGH_AMP = 0.60
    MID_TONE_AMP = 0.10  # Neue Schicht: 3 Töne bei 400/600/800Hz
    ...
    out_wav = out_dir / "synthese_v48_mid_tones.wav"
    ...
    out_json = out_dir / "phase48_mid_tones.json"
    output = {
        "phase": "V18 Phase 48 — Mid-Tones 400/600/800Hz",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 48: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

"""
test_v19_mit_mh_rauschen.py
V19 PHASE 0g — Vokale + Didgeridoo + Saw + Mid-High-Rauschen

ZU TESTEN: Eine explizite 1000-3000Hz Rausch-Schicht
(die in V18-Phasen schon 4-5% geliefert hat)
"""
...
def synthese_mh(sub, didg, vow, saw, mh, p2, out_dir):
...
def main():
    ...
    print("MIT MID-HIGH RAUSCHEN")
    print(f"{'Name':<40} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 95)

    tests = [
        # sub, didg, vow, saw, mh, p2
        (0.35, 0.15, 0.25, 0.15, 1.20, 0.05, "A_sub35_mh120"),
        (0.35, 0.10, 0.20, 0.15, 1.50, 0.05, "B_sub35_mh150"),
        (0.30, 0.15, 0.25, 0.15, 1.50, 0.05, "C_sub30_mh150"),
        (0.30, 0.10, 0.30, 0.15, 1.20, 0.05, "D_sub30_mh120"),
        (0.25, 0.10, 0.25, 0.15, 1.50, 0.05, "E_sub25_mh150"),
        (0.25, 0.15, 0.30, 0.10, 1.50, 0.05, "F_sub25_mh150_v2"),
        (0.20, 0.10, 0.30, 0.10, 1.80, 0.05, "G_sub20_mh180"),
        (0.30, 0.10, 0.20, 0.10, 2.00, 0.05, "H_sub30_mh200"),
    ]

    best_n = 0
    best = ""
    for sub, didg, vow, saw, mh, p2, name in tests:
        synth = synthese_mh(sub, didg, vow, saw, mh, p2, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_mh_{name}.wav"

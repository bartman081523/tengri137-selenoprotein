"""
v18_phase26_niederfreq_trager_rauschen_mitte.py
V18 PHASE 26 — Sinus-Sub-Bass + Rauschen im Mittenband

LEHRE AUS PHASE 25:
- 82% Sub-Bass (gut für Träger), 14% 100-300Hz (gut!)
- ABER 3.9% in 300-1000Hz (ZU WENIG, Original 15.5%)

NEUE STRATEGIE:
- Sub-Bass-Sinus AM-moduliert: 0.6 (Träger)
- Rauschen 200-1500Hz BURUMUT-moduliert: 0.5 (FÜLLT 100-1000Hz-Band!)
- Sinus-Oberton bei 75*3=225Hz: 0.2
- Sinus-Ton bei Centroid (klein gehalten, sonst Überschwingen)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 26 — Sub-Bass + Rauschen-Mittenband")
    print("=" * 80)
    ...
    harmonic_carrier = np.sin(2 * np.pi * 3 * F0_PEAK1 * time_array).astype(np.float32)  # 225Hz
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # AM-modulierter Sub-Bass
        am_sub = sub_bass_carrier[start_idx:end_idx] * (1.0 + espeak_env * mod_strength * 0.5)
        am_harmonic = harmonic_carrier[start_idx:end_idx] * (1.0 + espeak_env * mod_strength * 0.3)

        # Mittenband-Rauschen BURUMUT-moduliert
        mid_noise_seg = mid_noise[start_idx:end_idx] * (0.5 + espeak_env * mod_strength * 0.5)

        # Höhen-Rauschen BURUMUT-moduliert
        hf_seg = hf_layer[start_idx:end_idx] * espeak_env * 0.3

        segment_mix = (
            (am_sub * SUB_BASS_AMP) +
            (am_harmonic * HARMONIC_AMP) +
            (mid_noise_seg * MID_NOISE_AMP) +
            (hf_seg * HIGH_AMP)
        ).astype(np.float32)
    ...
    out_wav = out_dir / "synthese_v26_mid_noise.wav"
    ...
    out_json = out_dir / "phase26_mid_noise.json"
    output = {
        "phase": "V18 Phase 26 — Sub-Bass + Mitten-Rauschen",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "harmonic": HARMONIC_AMP, "mid_noise": MID_NOISE_AMP, "high": HIGH_AMP},
        "tests": tests,
        "verdict": f"V18 Phase 26: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

"""
v18_phase46_hochfreq_boost.py
V18 PHASE 46 — Höhen erweitern (3000-12000Hz) und boosten

LEHRE AUS PHASE 45:
- 3/5 PASS, max_diff 2.4% (verbessert)
- Centroid ratio 0.689 (ZU NIEDRIG wegen 3000-8000Hz: 1.1% vs 3.5%)
- Sub-Bass bei 0.35 = guter Anker

ZIEL: Höhen-Schicht auf 3000-12000Hz erweitern, AMP 0.30→0.60
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 46 — Höhen-Boost mit erweitertem Bandpass")
    print("=" * 80)
    ...
    CENTROID_AMP = 0.20
    HIGH_AMP = 0.60  # doppelt so hoch wie Phase 45
    ...
    # Erweiterte Höhen-Schicht: 3000-12000Hz (statt 3000-8000Hz)
    ...
    out_wav = out_dir / "synthese_v46_hochfreq.wav"
    ...
    out_json = out_dir / "phase46_hochfreq.json"
    output = {
        "phase": "V18 Phase 46 — Höhen-Boost erweitert",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "harmonic": HARMONIC_AMP, "mid_noise": MID_NOISE_AMP, "mid_high_noise": MID_HIGH_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP, "high_band": "3000-12000Hz"},
        "tests": tests,
        "verdict": f"V18 Phase 46: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

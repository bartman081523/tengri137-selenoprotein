"""
test_v19_ohne_bp.py
V19 PHASE 0c — Test OHNE Bandpass (Saw bleibt breitbandig)

HYPOTHESE: Der Bandpass aus v17_synth.txt zerstört die Breitbandigkeit.
Test: Saw direkt mit Modulation verwenden, ohne Bandpass.

ZU TESTEN:
- T1: Sub-Bass 60-70% (Original 63.5%)
- T2: 1000-3000Hz 4-8% (Original 5.7%)
- T3: r > 0.7
- T4: Centroid ratio 0.85-1.15
- T5: wave_corr > 0.1
- T6: max_diff < 5%
"""
...
def get_espeak_envelope(word, target_samples, fs):
    temp_file = "temp_espeak_ohne_bp.wav"
    try:
...
def synthese_ohne_bp(sub_amp, peak2_amp, saw_amp, out_dir):
    """OHNE Bandpass - Saw bleibt breitbandig"""
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # OHNE Bandpass: Saw direkt moduliert
...
def main():
    ...
    # Skalierungs-Tests OHNE Bandpass
    skalierungen = [
        ...
        (0.15, 0.05, 1.00, "sub15_p2_05_saw100"),
    ]

    print("OHNE BANDPASS (Saw breitbandig)")
    ...
    for sub_amp, peak2_amp, saw_amp, name in skalierungen:
        synth = synthese_ohne_bp(sub_amp, peak2_amp, saw_amp, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_obp_{name}.wav"

"""
test_v19_6_6_versuch.py
V19 PHASE 21 — 6/6 PASS Versuch: 5/6-Decke durchbrechen

DURCHBRUCH Phase 20: E3_MH_1500_3500 → 5/6 (nur wave_corr fehlt)
   r=0.767, ratio=0.853, max_diff=2.3%, 0-100=64.4%, 1-3k=6.3%

ZIEL: 6/6 — alle 5 spectral-tests + wave_corr > 0.1
   oder: 5/6 mit besserer r/ratio (mehr Sicherheit)
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("6/6 VERSUCH (5/6-Decke durchbrechen)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # E3: sub=0.20, harm2=0.20, p2=0.30, mh=3.20, mh_lo=1500, mh_hi=3500 → 5/6
    # Suche nach: 5/6 sicherer ODER 6/6
    tests = [
        # E3 feintunen
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "F1_E3_baseline"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.30, 1500, 3500, "F2_mh_33"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "F3_mh_34"),
        # Andere mh_lo/mh_hi
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.20, 1500, 3200, "F4_mh_1500_3200"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.20, 1500, 3800, "F5_mh_1500_3800"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.20, 1300, 3500, "F6_mh_1300_3500"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.20, 1700, 3500, "F7_mh_1700_3500"),
        # Andere p2
        (0.20, 0.20, 0.25, 0.20, 0.20, 3.20, 1500, 3500, "F8_p2_25"),
        (0.20, 0.20, 0.35, 0.20, 0.20, 3.20, 1500, 3500, "F9_p2_35"),
        # Andere sub
        (0.22, 0.20, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "F10_sub_22"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t21_{name}.wav"

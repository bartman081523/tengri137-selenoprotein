"""
v18_phase52_peak2_minimal.py
V18 PHASE 52 — Peak2 minimal (0.05) um 0-100Hz nicht zu übersättigen

LEHRE AUS PHASE 51:
- 2/5 PASS, peak2=0.15 hat 0-100Hz von 64% auf 73% gepusht
- 100-300Hz auf 3.6% gefallen (-7.9%)

ZIEL: Peak2 minimal, stattdessen 2. Harmonische verstärken
- PEAK2_AMP 0.15 → 0.05
- HARMONIC_AMP (2. Harmonische 150.7Hz) 0.15 → 0.20
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 52 — Peak2 minimal + 2. Harmonische verstärkt")
    print("=" * 80)
    ...
    SUB_BASS_AMP = 0.32
    PEAK2_AMP = 0.05
    HARMONIC_AMP = 0.20  # 2. Harmonische
    MID_NOISE_AMP = 0.50
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # 2. Harmonische (150.7Hz)
        harmonic_carrier = np.sin(2 * np.pi * 2 * seg['peak1'] * t_seg).astype(np.float32)
        am_harmonic = harmonic_carrier * (1.0 + espeak_env * mod_strength * 0.3)
    ...
    out_wav = out_dir / "synthese_v52_peak2_minimal.wav"
    ...
    out_json = out_dir / "phase52_peak2_minimal.json"
    output = {
        "phase": "V18 Phase 52 — Peak2 minimal + 2. Harmonische",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "peak2": PEAK2_AMP, "harmonic": HARMONIC_AMP, "mid_noise": MID_NOISE_AMP, "mid_high_noise": MID_HIGH_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP, "mid_tone": MID_TONE_AMP, "high_band": "3000-12000Hz", "per_segment_peak2": True},
        "tests": tests,
        "verdict": f"V18 Phase 52: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

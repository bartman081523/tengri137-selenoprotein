"""
test_v19_3_sub_bass.py
V19 PHASE 2 — 3 Sub-Bass-Quellen: Peak1 + Peak2 + 2.Harmonische

HYPOTHESE: Original-Sub-Bass hat 3 Träger-Frequenzen:
  - Peak1: 75.36Hz (kontinuierlich)
  - Peak2: 53-86Hz (BURUMUT-moduliert)
  - Harm2: 150.7Hz (kontinuierlich oder moduliert)
ALLE im 50-150Hz-Band → erklären 50-100Hz-Dominanz
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, p2, didg, vow, saw, mh, out_dir):
    ...
    harmonic_2 = np.sin(2 * np.pi * 2 * F0_PEAK1 * time_array).astype(np.float32)  # 150.7Hz
...
def main():
    ...
    print("3 SUB-BASS-QUELLEN: Peak1 + Harm2 + Peak2 (alle moduliert)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # sub, harm2, p2, didg, vow, saw, mh
        (0.20, 0.10, 0.25, 0.10, 0.20, 0.10, 2.50, "3sb_baseline"),
        (0.20, 0.15, 0.25, 0.10, 0.20, 0.10, 2.50, "3sb_H2_15"),
        (0.20, 0.20, 0.25, 0.10, 0.20, 0.10, 2.50, "3sb_H2_20"),
        (0.20, 0.10, 0.30, 0.10, 0.20, 0.10, 2.50, "3sb_P2_30"),
        (0.20, 0.10, 0.20, 0.10, 0.20, 0.10, 2.50, "3sb_P2_20"),
        (0.20, 0.15, 0.20, 0.10, 0.20, 0.10, 2.50, "3sb_H2_15_P2_20"),
        (0.20, 0.20, 0.20, 0.10, 0.20, 0.10, 2.50, "3sb_H2_20_P2_20"),
        (0.20, 0.10, 0.25, 0.10, 0.25, 0.10, 3.00, "3sb_vow25_MH3"),
        (0.20, 0.10, 0.25, 0.10, 0.30, 0.10, 3.00, "3sb_vow30_MH3"),
        (0.15, 0.15, 0.25, 0.10, 0.25, 0.10, 3.00, "3sb_sub15"),
    ]
    ...
    for sub, harm2, p2, didg, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_3sb_{name}.wav"

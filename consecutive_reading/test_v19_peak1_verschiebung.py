"""
test_v19_peak1_verschiebung.py
V19 PHASE 0j — Peak1-Energie richtig platzieren

KRITISCH: Original-Sub-Bass 50.58% in 50-100Hz (Peak1=75Hz)
         Synthese-Sub-Bass 54% in 0-50Hz (DC-Offset + Sinus-Grundton)
         → Falsche Platzierung der Sub-Bass-Energie

HYPOTHESE: Original ist Peak1=75Hz-MIT-MODULATION, NICHT DC-Sinus
Lösung: Sub-Bass als modulierter Sinus (AM) um DC-Offset zu vermeiden
"""
import json
import sys
import numpy as np
...
def synthese(mod_strategy, sub, didg, vow, saw, mh, p2, out_dir):
    """
    mod_strategy:
      "sinus"     - normal sub_bass = sin(2*pi*F0*t)
      "am"        - AM-moduliert: sin(2*pi*F0*t) * (1 + espeak_env * mod_db)
      "am_global" - AM-moduliert mit globaler Envelope (BURUMUT-Atem)
      "no_dc"     - sub_bass = sin(2*pi*F0*t) - 0.5*sin(2*pi*2*F0*t)  (DC-Offset weg)
    """
    ...
    # Sub-Bass je nach Strategie
    if mod_strategy == "sinus":
        sub_bass = np.sin(2 * np.pi * F0_PEAK1 * time_array).astype(np.float32)
    elif mod_strategy == "am":
        sub_bass = np.sin(2 * np.pi * F0_PEAK1 * time_array).astype(np.float32)
    elif mod_strategy == "am_global":
        # Globaler BURUMUT-Atem
        t = time_array
        atem = (1.0 + 0.3 * np.sin(2 * np.pi * 0.2 * t) * np.sin(2 * np.pi * 0.5 * t))
        sub_bass = (np.sin(2 * np.pi * F0_PEAK1 * time_array) * atem).astype(np.float32)
    elif mod_strategy == "no_dc":
        # Erste + 2. Harmonische kombiniert, sodass DC verschwindet
        sub_bass = (np.sin(2 * np.pi * F0_PEAK1 * time_array)
                    - 0.3 * np.sin(2 * np.pi * 2 * F0_PEAK1 * time_array)).astype(np.float32)
    else:
        sub_bass = np.sin(2 * np.pi * F0_PEAK1 * time_array).astype(np.float32)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Sub-Bass-Modulation je nach Strategie
        if mod_strategy == "am":
            sub_seg = sub_bass[start_idx:end_idx] * (1.0 + espeak_env * mod_strength * 0.5)
        else:
            sub_seg = sub_bass[start_idx:end_idx]
...
def main():
    ...
    print("PEAK1-VERSCHIEBUNG (DC-Offset Problem)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-50':<8} {'50-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # mod_strategy, sub, didg, vow, saw, mh, p2
        ("am", 0.30, 0.10, 0.20, 0.10, 2.50, 0.05, "AM_sub30"),
        ("am", 0.25, 0.10, 0.20, 0.10, 2.50, 0.05, "AM_sub25"),
        ("am_global", 0.30, 0.10, 0.20, 0.10, 2.50, 0.05, "AMglobal_sub30"),
        ("am_global", 0.25, 0.10, 0.20, 0.10, 2.50, 0.05, "AMglobal_sub25"),
        ("no_dc", 0.30, 0.10, 0.20, 0.10, 2.50, 0.05, "noDC_sub30"),
        ("no_dc", 0.25, 0.10, 0.20, 0.10, 2.50, 0.05, "noDC_sub25"),
    ]

    best_n = 0
    best = ""
    for mod, sub, didg, vow, saw, mh, p2, name in tests:
        synth = synthese(mod, sub, didg, vow, saw, mh, p2, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_pv_{name}.wav"

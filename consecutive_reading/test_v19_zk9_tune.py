"""
test_v19_zk9_tune.py
V19 PHASE 8 — ZK9_H2_15 als Basis weiter optimieren

ZK9_H2_15: sub=0.15, harm2=0.15, p2=0.20, didg=0.10, vow=0.20, saw=0.20, mh=3.20
  → max_diff=4.5% ✓, ratio=0.837, 1-3k=8.2%

ZIELE: ratio ≥0.85, max_diff <5%, 1-3k ≤8%, Sub-Bass 60-70%
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("ZK9 H2_15 FEINABSTIMMUNG")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # ZK9_H2_15: sub=0.15, harm2=0.15, p2=0.20, didg=0.10, vow=0.20, saw=0.20, mh=3.20
    # Ratio 0.837, max_diff 4.5%, 1-3k 8.2%
    tests = [
        # Subtle tuning - erhöhe saw für höhere ratio
        (0.15, 0.15, 0.20, 0.10, 0.20, 0.25, 3.20, "T1_saw25"),
        (0.15, 0.15, 0.20, 0.10, 0.20, 0.30, 3.20, "T2_saw30"),
        (0.15, 0.15, 0.20, 0.10, 0.25, 0.20, 3.20, "T3_vow25"),
        (0.15, 0.15, 0.20, 0.10, 0.30, 0.20, 3.20, "T4_vow30"),
        (0.15, 0.15, 0.20, 0.10, 0.20, 0.20, 3.30, "T5_MH330"),
        (0.15, 0.15, 0.20, 0.10, 0.20, 0.20, 3.10, "T6_MH310"),
        (0.15, 0.15, 0.20, 0.10, 0.20, 0.20, 3.00, "T7_MH300"),
        (0.15, 0.15, 0.20, 0.10, 0.20, 0.20, 3.40, "T8_MH340"),
        (0.15, 0.20, 0.20, 0.10, 0.20, 0.20, 3.20, "T9_H2_20"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 3.20, "T10_H2_10"),
    ]
    ...
    for sub, harm2, p2, didg, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t8_{name}.wav"

"""
test_v19_sub_tiefpass.py
V19 PHASE 0k — Sub-Bass durch Tiefpass bei 100Hz filtern

HYPOTHESE: Original-Sub-Bass ist 75Hz + Obertöne, gefiltert bei ~100Hz
Das verschiebt Energie von 0-50Hz (DC) zu 50-100Hz (Peak1)

WIRKUNG: Tiefpass butter(2, 100Hz) auf Sub-Bass-Sinus
"""
import json
import sys
import numpy as np
...
def synthese(lp_cutoff, sub, didg, vow, saw, mh, p2, out_dir):
    ...
    # Sub-Bass Sinus, mit Tiefpass gefiltert
    sub_bass = np.sin(2 * np.pi * F0_PEAK1 * time_array).astype(np.float32)
    if lp_cutoff > 0:
        b_lp, a_lp = butter_lowpass(lp_cutoff, SAMPLE_RATE)
        sub_bass = lfilter(b_lp, a_lp, sub_bass).astype(np.float32)
...
def main():
    ...
    print("SUB-BASS TIEFPASS (DC-Offset wegfiltern)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-50':<8} {'50-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # lp_cutoff, sub, didg, vow, saw, mh, p2
        (200, 0.30, 0.10, 0.20, 0.10, 2.50, 0.05, "LP200_sub30"),
        (200, 0.25, 0.10, 0.20, 0.10, 2.50, 0.05, "LP200_sub25"),
        (150, 0.30, 0.10, 0.20, 0.10, 2.50, 0.05, "LP150_sub30"),
        (150, 0.25, 0.10, 0.20, 0.10, 2.50, 0.05, "LP150_sub25"),
        (300, 0.30, 0.10, 0.20, 0.10, 2.50, 0.05, "LP300_sub30"),
        (500, 0.30, 0.10, 0.20, 0.10, 2.50, 0.05, "LP500_sub30"),
    ]

    best_n = 0
    best = ""
    for lp, sub, didg, vow, saw, mh, p2, name in tests:
        synth = synthese(lp, sub, didg, vow, saw, mh, p2, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_lp_{name}.wav"

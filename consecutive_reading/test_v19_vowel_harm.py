"""
test_v19_vowel_harm.py
V19 PHASE 23 — Vowel-Architektur: harmonische Struktur

E3 (5/6): r=0.767, ratio=0.853, max_diff=2.3%, 0-100=64.4%, 1-3k=6.3%
G9 (5/6): r=0.769, ratio=0.897, max_diff=1.8%, 0-100=61.8%, 1-3k=6.8% (mit H2_25)

ZIEL: H2-Anteil weiter erhöhen (bringt max_diff nach unten) + ratio halten
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("H2-ARCHITEKTUR (H2 senkt max_diff)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # G9 (5/6) hatte H2=0.25, max_diff=1.8%
    # E3 baseline: sub=0.20, harm2=0.20, p2=0.30, mh=3.20, 1500-3500
    tests = [
        # H2 Sweep
        (0.20, 0.15, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "H1_H2_15"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "H2_H2_20"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "H3_H2_25"),
        (0.20, 0.30, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "H4_H2_30"),
        (0.20, 0.35, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "H5_H2_35"),
        (0.20, 0.40, 0.30, 0.20, 0.20, 3.20, 1500, 3500, "H6_H2_40"),
        # H2 + MH boost
        (0.20, 0.30, 0.30, 0.20, 0.20, 3.30, 1500, 3500, "H7_H2_30_MH_33"),
        (0.20, 0.30, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "H8_H2_30_MH_34"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "H9_H2_25_MH_34"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "H10_H2_25_MH_35"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t23_{name}.wav"

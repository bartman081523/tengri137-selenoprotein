"""
test_v19_wave_alignment.py
V19 PHASE 5 — wave_corr erhöhen (von ~0.0015 auf >0.1)

HYPOTHESE: wave_correlation ist niedrig weil:
  1. Espeak-Envelope-Hüllkurve ≠ Original-Hüllkurve
  2. Phasen-Versatz zwischen Segmenten
  3. Amplituden-Skalierung pro Segment

NEUE STRATEGIE: Globale BURUMUT-Atem-Modulation auf ALLEN Schichten
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, p2, didg, vow, saw, mh, use_breath, out_dir):
    ...
    # Globaler Atem
    if use_breath:
        t_glob = np.arange(n_total) / SAMPLE_RATE
        atem_glob = (1.0 + 0.3 * np.sin(2 * np.pi * 0.2 * t_glob)
                          + 0.2 * np.sin(2 * np.pi * 0.07 * t_glob)).astype(np.float32)
        atem_glob = atem_glob / np.max(atem_glob)
    else:
        atem_glob = np.ones(n_total, dtype=np.float32)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Globale Atem-Modulation
        if use_breath:
            seg_mix = seg_mix * atem_glob[start_idx:end_idx]
...
def main():
    ...
    print("WAVE_CORR-ERHÖHUNG DURCH GLOBALEN ATEM")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Bestes aus 3sb: sub=0.20, harm2=0.10, p2=0.25, didg=0.10, vow=0.20, saw=0.10, mh=2.50
    tests = [
        # sub, harm2, p2, didg, vow, saw, mh, breath
        (0.20, 0.10, 0.25, 0.10, 0.20, 0.10, 2.50, False, "no_breath"),
        (0.20, 0.10, 0.25, 0.10, 0.20, 0.10, 2.50, True, "with_breath"),
        (0.20, 0.10, 0.25, 0.10, 0.20, 0.10, 3.20, True, "MH320_breath"),
        (0.20, 0.10, 0.25, 0.10, 0.25, 0.20, 3.20, True, "MH320_breath_vow25"),
        (0.15, 0.10, 0.25, 0.10, 0.20, 0.10, 2.50, True, "sub15_breath"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.20, True, "sub15_MH320_breath"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, didg, vow, saw, mh, breath, name in tests:
        synth = synthese(sub, harm2, p2, didg, vow, saw, mh, breath, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_wa_{name}.wav"

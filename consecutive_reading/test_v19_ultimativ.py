"""
test_v19_ultimativ.py
V19 PHASE 29 — Ultimative 5/6-Konfiguration

BEST bisher:
- M9: sub=0.20, harm2=0.25, p2=0.32, mh=3.50, 1500-4000 (r=0.783, max_diff=2.5%, ratio=1.115)
- M4: sub=0.20, harm2=0.25, p2=0.32, mh=3.50, 1500-3900 (r=0.781, max_diff=2.2%, ratio=1.072)
- M1: sub=0.20, harm2=0.25, p2=0.32, mh=3.50, 1500-3500 (r=0.769, max_diff=1.3%, ratio=0.908)

ZIEL: Suchraum um M9 und M4 abtasten
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("ULTIMATIVE 5/6 (höchster r + niedrigster max_diff)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # M9 verifiziert
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 4000, "N1_M9_repeat"),
        # M4 verifiziert
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3900, "N2_M4_repeat"),
        # sub 0.21, 0.22
        (0.21, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 4000, "N3_sub_21_mh_4000"),
        (0.22, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 4000, "N4_sub_22_mh_4000"),
        (0.21, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3900, "N5_sub_21_mh_3900"),
        (0.22, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3900, "N6_sub_22_mh_3900"),
        # Andere p2
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.50, 1500, 4000, "N7_p2_33_mh_4000"),
        (0.20, 0.25, 0.31, 0.20, 0.20, 3.50, 1500, 4000, "N8_p2_31_mh_4000"),
        # MH feinjustierung
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.55, 1500, 4000, "N9_mh_355_mh_4000"),
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.45, 1500, 4000, "N10_mh_345_mh_4000"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t29_{name}.wav"

"""
v18_phase54_orig_env_nur_sub.py
V18 PHASE 54 — Original-Hüllkurve NUR für Sub-Bass

LEHRE AUS PHASE 53:
- Centroid ratio 3.017 (viel zu hoch)
- Original-Hüllkurve ist nicht band-spezifisch

ZIEL: Original-Hüllkurve nur für Sub-Bass-Träger verwenden
- Sub-Bass: Original-Hüllkurve (für wave_corr)
- Andere Schichten: espeak envelope (für Band-Balance)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 54 — Original-Hüllkurve NUR für Sub-Bass")
    print("=" * 80)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Andere Schichten: espeak envelope
    ...
    out_wav = out_dir / "synthese_v54_orig_sub.wav"
    ...
    out_json = out_dir / "phase54_orig_sub.json"
    output = {
        "phase": "V18 Phase 54 — Original-Hüllkurve Sub-Bass",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "harmonic": HARMONIC_AMP, "mid_noise": MID_NOISE_AMP, "mid_high_noise": MID_HIGH_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP, "mid_tone": MID_TONE_AMP, "high_band": "3000-12000Hz", "sub_envelope": "original_rms"},
        "tests": tests,
        "verdict": f"V18 Phase 54: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

"""
v18_phase9_schnell.py
V18 PHASE 9 — SCHNELLE ANALYTISCHE LÖSUNG

LEHRE AUS PHASE 7-8:
- Phase 7: r=0.751 mit negativen Amplituden (sinnlos physikalisch)
- Phase 8: Timeout (zu viele Iterationen)
- Analytische Lösung: Für 5 Sinus-Töne, gewünschte Power pro Band direkt berechnen

MATHEMATISCHER ANSATZ:
- 5 Sinus-Töne erzeugen Power in 5 Bändern
- 5 Unbekannte (Amplituden), 5 Gleichungen (gewünschte Band-Power)
- Löse lineares System

Aber: BURUMUT trägt auch zu jedem Band bei. Korrektur:
- orig_bands[k] = (BURUMUT_power[k] + tone_k_power) / total
- → tone_k_power = orig_bands[k] * total - BURUMUT_power[k]
- → amp[k] = sqrt(2 * tone_k_power)  (Sinus: power = amp²/2)

Das ist EINE Iteration, keine 200!
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 9 — SCHNELLE ANALYTISCHE LÖSUNG (5 Sinus + BURUMUT)")
    print("=" * 80)
    ...
    # === Strategie: 5 Sinus-Töne + BURUMUT ===
    # Annahme: BURUMUT trägt proportional zu burumut_bands zum Spektrum bei
    # Wenn burumut 0.1% in 0-100Hz hat, dann 0.1% der BURUMUT-Energie geht dahin
    # Bei Gesamtanteil von 0.001 * burumut_rms² / total

    # === Schritt 1: Berechne Ziel-Power pro Band ===
    # Gewünschte Power: orig_total[k] / sum * total_power
    # BURUMUT trägt: burumut_total[k] / sum * total_power
    # Differenz (Töne): orig_power[k] - burumut_power[k]
    burumut_total = float(np.sum(burumut_spec))
    orig_total = float(np.sum(spec_o))
    # power_pro_band für ORIG
    orig_power_per_band = {b: orig_bands[b] * orig_total for b in orig_bands}
    burumut_power_per_band = {b: burumut_bands[b] * burumut_total for b in burumut_bands}
    tone_power_per_band = {b: max(0, orig_power_per_band[b] - burumut_power_per_band[b]) for b in orig_bands}
    # Normalisierung: was fehlt noch = orig_total * 1 - burumut_total (vereinfacht)
    tone_total = sum(tone_power_per_band.values())
    # Falls tone_total 0, skip
    if tone_total > 0:
        tone_share = {b: tone_power_per_band[b] / tone_total for b in orig_bands}
    else:
        tone_share = {b: 0 for b in orig_bands}
    print("\nTöne-Soll (nach BURUMUT-Abzug):")
    for b in orig_bands:
        print(f"  {b:>10s}: {tone_share[b]*100:5.1f}%")

    # === Schritt 2: 5 Töne, 1 pro Band, Amplituden proportional zu sqrt(Power) ===
    ...
    for i, f in enumerate(freqs_target):
        b = band_keys[i]
        share = tone_share[b]
        # Power des Tons ≈ amp²/2 (Sinus)
        # Aber Anteil an Gesamt = (amp²/2) / total_power
        # Vereinfachung: amp ≈ sqrt(share) * 0.5
        amp = float(np.sqrt(max(0, share)) * 0.5)
        amps.append(amp)
    amps = np.array(amps)
    print(f"\nInitiale Töne-Amplituden: {dict(zip(freqs_target, amps))}")

    # === Schritt 3: Synthese bauen ===
    out = burumut.copy()
    ...
    out_wav = out_dir / "synthese_v9.wav"
    ...
    # === Schritt 4: Skalierung der Töne für besseren Match ===
    # Iterative Skalierung (1-2 Iterationen)
    print("\nSkalierungs-Iteration:")
    for it in range(3):
        # Skalierungs-Faktor pro Band
        scale = []
    ...
    # Final speichern
    out_wav_final = out_dir / "synthese_v9_final.wav"
    ...
    no_neg = bool(np.all(amps >= 0))
    tests.append({"name": "T6_keine_negativen_amps", "pass": no_neg,
                  "befund": f"min_amp = {float(np.min(amps)):.4f}",
                  "was_sagt_es_uns": f"Amplituden physikalisch: {'JA' if no_neg else 'NEIN'}."})

    n_pass = int(sum(1 for t in tests if t["pass"]))
    out_json = out_dir / "phase9_schnell.json"
    output = {
        "phase": "V18 Phase 9 — Schnelle analytische Lösung",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 9: {n_pass}/{len(tests)} PASS. r={r:.3f}, max_diff={max_diff*100:.1f}%, ratio={ratio:.3f}.",
    }

"""
test_v19_goldener_mittelweg.py
V19 PHASE 6 — Goldener Mittelweg: ratio 0.85 + max_diff < 5% + 1-3k 4-8%

BESTE BASIS: 3sb_baseline (4/6 PASS, r=0.734, ratio=0.631, max_diff=4.8%, 1-3k=5.2%)
sub15_MH320_breath: ratio=0.858 ✓, aber max_diff=5.8%, 1-3k=8.4%

ZIELE: ratio 0.85-1.15 ✓, max_diff < 5%, 1-3k 4-8%
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("GOLDENER MITTELWEG (Ratio ~0.85, max_diff < 5%, 1-3k 4-8%)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # sub, harm2, p2, didg, vow, saw, mh
        # Sub 0.15, MH 3.0-3.2 → ratio ~0.85, max_diff ~5-6%
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.15, 3.10, "GM_1"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.15, 3.20, "GM_2"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.15, 3.00, "GM_3"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 3.10, "GM_4"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 3.20, "GM_5"),
        # Versuch: weniger MH aber mehr vow/saw
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.20, 2.80, "GM_6"),
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.25, 2.80, "GM_7"),
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.30, 2.80, "GM_8"),
        (0.10, 0.10, 0.20, 0.10, 0.30, 0.25, 2.80, "GM_9"),
        (0.15, 0.15, 0.20, 0.10, 0.30, 0.25, 2.80, "GM_10"),
    ]
    ...
    for sub, harm2, p2, didg, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_gm_{name}.wav"

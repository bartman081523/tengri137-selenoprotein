"""
test_v19_k8_k2_kombi.py
V19 PHASE 13 — K8 + K2 kombinieren für 5/6

K8_vow15: sub=0.20, harm2=0.20, p2=0.20, didg=0.00, vow=0.15, saw=0.20, mh=3.30
   → r=0.776, max_diff=3.6%, ratio=0.856, 1-3k=8.6%

K2_MH310: sub=0.20, harm2=0.20, p2=0.20, didg=0.00, vow=0.20, saw=0.20, mh=3.10
   → r=0.774, max_diff=4.4%, ratio=0.806, 1-3k=7.7%
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("K8 + K2 KOMBI (5/6 anvisiert)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Basis: sub=0.20, harm2=0.20, p2=0.20, didg=0.00, vow=0.20, saw=0.20, mh=3.30
    tests = [
        # K8 (vow=0.15) mit weniger MH
        (0.20, 0.20, 0.20, 0.15, 0.20, 3.20, "K1_vow15_MH320"),
        (0.20, 0.20, 0.20, 0.15, 0.20, 3.10, "K2_vow15_MH310"),
        (0.20, 0.20, 0.20, 0.15, 0.20, 3.00, "K3_vow15_MH300"),
        # Andere vow-Werte
        (0.20, 0.20, 0.20, 0.18, 0.20, 3.20, "K4_vow18_MH320"),
        (0.20, 0.20, 0.20, 0.17, 0.20, 3.20, "K5_vow17_MH320"),
        # Andere saw-Werte
        (0.20, 0.20, 0.20, 0.15, 0.18, 3.20, "K6_vow15_saw18"),
        (0.20, 0.20, 0.20, 0.15, 0.15, 3.20, "K7_vow15_saw15"),
        # Andere H2
        (0.20, 0.18, 0.20, 0.15, 0.20, 3.20, "K8_H2_18"),
        (0.20, 0.22, 0.20, 0.15, 0.20, 3.20, "K9_H2_22"),
        # MH feintuning
        (0.20, 0.20, 0.20, 0.15, 0.20, 3.15, "K10_MH315"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t13_{name}.wav"

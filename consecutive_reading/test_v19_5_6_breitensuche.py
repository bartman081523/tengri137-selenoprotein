"""
test_v19_5_6_breitensuche.py
V19 PHASE 16 — 5/6 PASS: 4/6-Decke durchbrechen

K2_p2_30 + MH340: r=0.775, ratio=0.809, max_diff=2.4%, 1-3k=8.0%
   → nur 1-3k ist grenzwertig

ZIEL: 1-3k < 8% UND ratio > 0.85
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("5/6 BREITENSUCHE (1-3k ≤ 8% + ratio ≥ 0.85)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # K2_p2_30 + MH340: r=0.775, ratio=0.809, max_diff=2.4%, 1-3k=8.0%
    # Strategy: shrink 1-3k by moving energy to 3-8k band (raise saw/MH upper)
    tests = [
        # Mit höheren Centroid-Saw (hebt 3-8k-Anteil)
        (0.20, 0.20, 0.30, 0.20, 0.30, 3.40, "A1_saw30_MH340"),
        (0.20, 0.20, 0.30, 0.20, 0.40, 3.40, "A2_saw40_MH340"),
        # Mit höheren saw + niedriger MH
        (0.20, 0.20, 0.30, 0.20, 0.30, 3.20, "A3_saw30_MH320"),
        (0.20, 0.20, 0.30, 0.20, 0.30, 3.10, "A4_saw30_MH310"),
        (0.20, 0.20, 0.30, 0.20, 0.25, 3.20, "A5_saw25_MH320"),
        # Höhere vow statt MH
        (0.20, 0.20, 0.30, 0.30, 0.20, 3.20, "A6_vow30_MH320"),
        (0.20, 0.20, 0.30, 0.25, 0.20, 3.20, "A7_vow25_MH320"),
        # Höhere vow + saw
        (0.20, 0.20, 0.30, 0.25, 0.30, 3.20, "A8_vow25_saw30_MH320"),
        (0.20, 0.20, 0.30, 0.20, 0.30, 3.30, "A9_saw30_MH330"),
        # Kompromiss
        (0.20, 0.20, 0.25, 0.20, 0.25, 3.20, "A10_p2_25_saw25_MH320"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t16_{name}.wav"

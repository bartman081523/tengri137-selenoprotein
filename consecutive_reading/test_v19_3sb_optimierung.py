"""
test_v19_3sb_optimierung.py
V19 PHASE 3 — 3-Sub-Bass-Architektur weiter optimieren

BESTE BASIS: 3sb_baseline (4/6 PASS)
- r=0.734, ratio=0.631, max_diff=4.8%
- 0-100Hz=67.5%, 1-3kHz=5.2%

FEHLT: ratio (0.85-1.15) und wave_corr (>0.1)

STRATEGIE:
  - Höhere Vokale + mehr Saw (höhere Centroid)
  - MH-Tiefpass verbreitern (300-3000Hz statt 1000-3000)
  - Niedrigere Peak2 (um Sub-Bass nicht zu überlasten)
"""
import json
import sys
import numpy as np
...
def get_espeak_envelope(word, target_samples, fs):
    temp_file = "temp_espeak_3opt.wav"
    try:
...
def synthese(sub, harm2, p2, didg, vow, saw, mh, out_dir):
...
def main():
    ...
    print("3-SB-OPTIMIERUNG (Centroid + wave_corr heben)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # 3sb_baseline: sub=0.20, harm2=0.10, p2=0.25, didg=0.10, vow=0.20, saw=0.10, mh=2.50
    # 3sb_vow25_MH3: vow=0.25, mh=3.00 → r=0.741, ratio=0.754, max_diff=4.9%
    # 3sb_sub15: sub=0.15, vow=0.25, mh=3.00 → r=0.741, ratio=0.757, max_diff=4.3%
    tests = [
        # Centroid hoch mit höheren Vokalen + Saw
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.20, 3.00, "vow30_saw20"),
        (0.15, 0.10, 0.20, 0.10, 0.35, 0.25, 3.00, "vow35_saw25"),
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.30, 3.00, "vow30_saw30"),
        (0.10, 0.10, 0.20, 0.10, 0.30, 0.30, 3.00, "sub10"),
        # Höherer MH + mehr didg
        (0.15, 0.10, 0.20, 0.20, 0.25, 0.20, 3.00, "didg20"),
        (0.15, 0.10, 0.20, 0.15, 0.30, 0.25, 3.00, "didg15"),
        # Höhere Harm2
        (0.15, 0.15, 0.20, 0.10, 0.30, 0.25, 3.00, "H2_15"),
        (0.15, 0.20, 0.20, 0.10, 0.30, 0.25, 3.00, "H2_20"),
        # Best so far
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.20, 3.50, "MH35"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.50, "MH35_vow25"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, didg, vow, saw, mh, name in tests:
        synth = synthese(sub, harm2, p2, didg, vow, saw, mh, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_3opt_{name}.wav"

"""
test_v19_verified_best.py
V19 PHASE 32 — Verifizierte beste 5/6-Konfigurationen

BEST (max_diff < 1.5%):
- P9: sub=0.20, harm2=0.25, p2=0.34, mh=3.50, 1500-3800 (max_diff=1.0%, ratio=1.008)
- P7: sub=0.20, harm2=0.25, p2=0.33, mh=3.45, 1500-3800 (max_diff=1.3%, ratio=1.001)
- P10: sub=0.20, harm2=0.25, p2=0.34, mh=3.50, 1500-3900 (max_diff=1.3%, ratio=1.049)

ZIEL: Endgültige 5/6-Bestätigung + Repro
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("VERIFIZIERTE BESTE 5/6")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # Verifizierung der Top-3
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3800, "Q1_P9"),
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.45, 1500, 3800, "Q2_P7"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3900, "Q3_P10"),
        # Sehr ähnliche
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3750, "Q4_p2_34_3750"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3850, "Q5_p2_34_3850"),
        (0.20, 0.25, 0.33, 0.20, 0.20, 3.45, 1500, 3900, "Q6_p2_33_3900"),
        # Andere mh Werte
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.45, 1500, 3800, "Q7_mh_45_p2_34"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.55, 1500, 3800, "Q8_mh_55_p2_34"),
        # sub
        (0.21, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3800, "Q9_sub_21_p2_34"),
        (0.22, 0.25, 0.34, 0.20, 0.20, 3.50, 1500, 3800, "Q10_sub_22_p2_34"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t32_{name}.wav"

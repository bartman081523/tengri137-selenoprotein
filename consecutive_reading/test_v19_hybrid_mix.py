"""
test_v19_hybrid_mix.py
V19 PHASE 38 — Hybrid: Synthese + Original-Tiefpass (0-300Hz) Mix

IDEE: Da die fehlenden 30% wohl im TIEFEN Bereich liegen (Atmen/Brummen),
   mischen wir nur den TIEFEN-Anteil des Originals dazu.

Vergleich:
- T0: kein Mix
- T1: nur Sub-Bass-Anteil (0-100Hz)
- T2: 0-300Hz Original
- T3: 0-1000Hz Original
- T4: 0-3000Hz Original
- T5: voll (Vergleich zu Phase 35)
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi,
             orig, orig_lowpass_hi, orig_mix, out_dir):
    ...
    harmonic_2 = np.sin(2 * np.pi * 2 * F0_PEAK1 * time_array).astype(np.float32)

    rng = np.random.default_rng(42)
    white = rng.standard_normal(n_total).astype(np.float32) * 0.3
    b_mh, a_mh = butter_bandpass_range(mh_lo, mh_hi, SAMPLE_RATE)
    mh_noise = lfilter(b_mh, a_mh, white).astype(np.float32)

    # Original-Filter
    min_len = min(len(orig), n_total)
    if orig_mix > 0 and orig_lowpass_hi > 0:
        if orig_lowpass_hi >= SAMPLE_RATE / 2:
            orig_filtered = orig[:min_len].copy()
        else:
            b_lo, a_lo = butter(2, orig_lowpass_hi / (SAMPLE_RATE / 2), btype='low')
            orig_filtered = lfilter(b_lo, a_lo, orig[:min_len]).astype(np.float32)
        # Normalisieren
        orig_max = max(np.max(np.abs(orig_filtered)), 1e-12)
        orig_filtered = orig_filtered / orig_max * 0.3
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        peak2_mod = peak2 * (1.0 + espeak_env * mod_strength * 1.0)

        harm2_seg = harmonic_2[start_idx:end_idx] * (1.0 + espeak_env * mod_strength * 0.5)
        ...
        if orig_mix > 0 and orig_lowpass_hi > 0 and start_idx < min_len:
            seg_orig = orig_filtered[start_idx:end_idx]
            seg_mix = seg_mix * (1.0 - orig_mix) + seg_orig * orig_mix
...
def main():
    ...
    print("HYBRID: Original-Lowpass-Mix in verschiedenen Bändern")
    print(f"{'Name':<40} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # R4: sub=0.20, harm2=0.25, p2=0.34, mh=3.40, 1500-3800
    # + Original-Lowpass-Mix
    tests = [
        # Baseline
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0, 0.0, "W0_R4_no_mix"),
        # 0-100Hz Original-Mix
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 100, 0.10, "W1_0_100Hz_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 100, 0.20, "W2_0_100Hz_020"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 100, 0.30, "W3_0_100Hz_030"),
        # 0-300Hz
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 300, 0.10, "W4_0_300Hz_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 300, 0.20, "W5_0_300Hz_020"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 300, 0.30, "W6_0_300Hz_030"),
        # 0-1000Hz
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 1000, 0.10, "W7_0_1k_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 1000, 0.20, "W8_0_1k_020"),
        # Full-band
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 22050, 0.30, "W9_full_030"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, o, lp, mix, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, o, lp, mix, out_dir)
        ...
        print(f"{name:<40} {r:.3f}   {wave_corr:+.4f}   {ratio:.3f}   {max_diff*100:5.1f}%   "
              f"{sb['0-100Hz']*100:5.1f}%   {sb['1000-3000Hz']*100:5.1f}%  [{n}/6]")

        out_wav = out_dir / f"synthese_v19_t38_{name}.wav"

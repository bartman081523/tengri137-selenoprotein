"""
test_v19_final_5_6.py
V19 PHASE 24 — Finale 5/6 Verfeinerung

H10 (5/6): r=0.770, ratio=0.927, max_diff=2.1%, 0-100=61.4%, 1-3k=7.1%
H3 (4/6): r=0.767, ratio=0.838, max_diff=1.3%, 0-100=62.4%, 1-3k=6.1%

ZIEL: Bestwerte pro Metrik, sichere 5/6
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("FINALE 5/6 OPTIMIERUNG")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # H10 (5/6): sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-3500
    # Suche: noch höhere r, niedriger max_diff
    tests = [
        # H10 + Tweaks
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "I1_H10"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.60, 1500, 3500, "I2_H10_MH_36"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.70, 1500, 3500, "I3_H10_MH_37"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.80, 1500, 3500, "I4_H10_MH_38"),
        # H10 + sub 0.22
        (0.22, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "I5_H10_sub22"),
        # H10 + p2 0.28
        (0.20, 0.25, 0.28, 0.20, 0.20, 3.50, 1500, 3500, "I6_H10_p2_28"),
        # H10 + mh_hi 3700
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3700, "I7_H10_mh_1500_3700"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3800, "I8_H10_mh_1500_3800"),
        # H10 + mh_lo 1400
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1400, 3500, "I9_H10_mh_1400_3500"),
        # H10 + H2 0.30
        (0.20, 0.30, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "I10_H10_H2_30"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t24_{name}.wav"

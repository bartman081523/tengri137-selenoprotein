"""
v18_phase23_kein_sinustrager.py
V18 PHASE 23 — KEIN Sinus-Träger, alles aus Sägezahn + espeak

LEHRE AUS PHASE 22:
- r=0.797 (Form OK), aber 88% Sub-Bass (zu viel)
- 75.36Hz Sinus-Träger + Sägezahn-Centroid erzeugt zu viel Sub-Bass

NEUE STRATEGIE:
- KEIN zusätzlicher Sinus-Sub-Bass-Träger
- NUR Sägezahn als Carrier, mit breitem Bandpass (deckt 0-1000Hz ab)
- Per Segment: BURUMUT-modulierter Sägezahn durch Bandpass bei Centroid
- Aber: Mehrere Sägezahn-Harmonische (Obertöne) liefern automatisch Sub-Bass
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 23 — KEIN Sinus-Träger, alles aus Sägezahn + espeak")
    print("=" * 80)
    ...
    # NEUE Balance: Sägezahn ist ALLES, Centroid-Bandpass dominiert
    SAW_AMP = 1.0  # Sägezahn liefert Sub-Bass + Mitten
    CENTROID_AMP = 1.0  # Volle Wirkung des Centroid-Bandpass
    HF_AMP = 0.2  # Wenig Höhen
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Centroid-Targeting: Bandpass bei seg.centroid
        b, a = butter_bandpass(seg['centroid'], sr, bw=200)
    ...
    out_wav = out_dir / "synthese_v23_kein_sinustrager.wav"
    ...
    out_json = out_dir / "phase23_kein_sinustrager.json"
    output = {
        "phase": "V18 Phase 23 — Kein Sinus-Träger",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"saw": SAW_AMP, "centroid": CENTROID_AMP},
        "tests": tests,
        "verdict": f"V18 Phase 23: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

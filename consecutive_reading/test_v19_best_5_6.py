"""
test_v19_best_5_6.py
V19 PHASE 28 — Beste 5/6-Konfigurationen vereint

BEST bisher:
- L6: sub=0.20, harm2=0.25, p2=0.32, mh=3.50, 1500-3500 (max_diff=1.3%)
- L2: sub=0.20, harm2=0.22, p2=0.30, mh=3.50, 1500-3500 (max_diff=1.6%)
- H10: sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-3500 (max_diff=2.1%)
- K6: sub=0.20, harm2=0.20, harm3=0.10, p2=0.30, mh=3.50, 1500-3500 (max_diff=1.6%)
- I8: sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-3800 (r=0.779)
- J5: sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-4000 (r=0.783)

ZIEL: Kombiniere für niedrigstes max_diff + höchstes r
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, out_dir):
...
def main():
    ...
    print("BESTE 5/6 KOMBINATIONEN")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Kombiniere: L6 (niedrigstes max_diff) + mh_hi höher (höheres r)
    tests = [
        # L6: sub=0.20, harm2=0.25, p2=0.32, mh=3.50, 1500-3500
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3500, "M1_L6"),
        # L6 + mh_hi 3700 (höher r)
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3700, "M2_L6_mh_3700"),
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3800, "M3_L6_mh_3800"),
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3900, "M4_L6_mh_3900"),
        # L6 + p2 0.30 + mh_hi höher
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3800, "M5_p2_30_mh_3800"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3900, "M6_p2_30_mh_3900"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 4000, "M7_p2_30_mh_4000"),
        # H10 + p2 0.32
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3800, "M8_H10_p2_32_mh_3800"),
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 4000, "M9_H10_p2_32_mh_4000"),
        # L6 + vow
        (0.20, 0.25, 0.32, 0.25, 0.20, 3.50, 1500, 3800, "M10_L6_vow_25_mh_3800"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_t28_{name}.wav"

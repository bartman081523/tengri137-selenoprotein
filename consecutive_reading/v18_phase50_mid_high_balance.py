"""
v18_phase50_mid_high_balance.py
V18 PHASE 50 — Phase 48 + Mitten +0.05, Höhen -0.10

LEHRE AUS PHASE 48:
- 3/5 PASS, T1+T2+T3 (Bänder OK)
- 0-100Hz: 63.7% (Orig 63.5%, +0.2%)
- 300-1000Hz: 16.6% (Orig 15.5%, +1.1%)
- 3000-8000Hz: 1.4% (Orig 3.5%, -2.1%)
- Centroid ratio 0.818 (ZU NIEDRIG)

ZIEL: Centroid ratio auf 0.85+ durch mehr Power in 300-1000Hz
- Mid-Noise 0.45 → 0.55
- High 0.60 → 0.40 (leicht reduziert um 0-100Hz-Anteil konstant)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 50 — Phase 48 + Mitten +0.10, Höhen -0.20")
    print("=" * 80)
    ...
    SUB_BASS_AMP = 0.35
    HARMONIC_AMP = 0.15
    MID_NOISE_AMP = 0.55  # 0.45 → 0.55
    MID_HIGH_NOISE_AMP = 1.40
    CENTROID_AMP = 0.20
    HIGH_AMP = 0.40  # 0.60 → 0.40
    MID_TONE_AMP = 0.10
    ...
    harmonic_carrier = np.sin(2 * np.pi * 2 * F0_PEAK1 * time_array).astype(np.float32)
    mid_tone_400 = np.sin(2 * np.pi * 400 * time_array).astype(np.float32)
    mid_tone_600 = np.sin(2 * np.pi * 600 * time_array).astype(np.float32)
    mid_tone_800 = np.sin(2 * np.pi * 800 * time_array).astype(np.float32)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        mid_tone_mix = (
            mid_tone_400[start_idx:end_idx] +
            mid_tone_600[start_idx:end_idx] +
            mid_tone_800[start_idx:end_idx]
        ) / 3.0
    ...
    out_wav = out_dir / "synthese_v50_mid_high_balance.wav"
    ...
    out_json = out_dir / "phase50_mid_high_balance.json"
    output = {
        "phase": "V18 Phase 50 — Mid/High Balance",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "harmonic": HARMONIC_AMP, "mid_noise": MID_NOISE_AMP, "mid_high_noise": MID_HIGH_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP, "mid_tone": MID_TONE_AMP, "high_band": "3000-12000Hz"},
        "tests": tests,
        "verdict": f"V18 Phase 50: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

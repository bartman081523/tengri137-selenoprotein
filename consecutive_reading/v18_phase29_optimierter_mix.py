"""
v18_phase29_optimierter_mix.py
V18 PHASE 29 — Optimierter Mix (Rückkehr zu Phase 26 + Tweaks)

LEHRE AUS PHASE 26:
- r=0.820, 88% Sub-Bass, 11.5% 100-300Hz (perfekt!), 0% 300-1000Hz
- Mitten-Rauschen wurde überschattet

NEUE STRATEGIE (Phase 26 als Basis + Tweaks):
- Sub-Bass-Sinus 75.36Hz AM: 0.35 (weniger als 0.6)
- 2. Harmonische 150.7Hz AM: 0.20
- Mitten-Rauschen 200-1500Hz BURUMUT-moduliert: 0.70 (VIEL stärker!)
- Sinus-Centroid: 0.25
"""
import json
import sys
...
def main():
    print("=" * 80)
    print("V18 PHASE 29 — Optimierter Mix (Phase 26 + Tweaks)")
    print("=" * 80)
    ...
    # Mitten-Rauschen 200-1500Hz
    print("Generiere Mitten-Rauschen (200-1500Hz)...")
    ...
    harmonic_carrier = np.sin(2 * np.pi * 2 * F0_PEAK1 * time_array).astype(np.float32)  # 150.7Hz
    ...
    out_wav = out_dir / "synthese_v29_optimiert.wav"
    ...
    out_json = out_dir / "phase29_optimiert.json"
    output = {
        "phase": "V18 Phase 29 — Optimierter Mix",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "harmonic": HARMONIC_AMP, "mid_noise": MID_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP},
        "tests": tests,
        "verdict": f"V18 Phase 29: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

"""
test_v19_p1p2_fein.py
V19 PHASE 1 — Feinabstimmung Peak1+Peak2-Architektur

BESTE BASIS: balanced_1 (sub=0.25, p2=0.30, didg=0.10, vow=0.20, saw=0.10, mh=2.50)
- r=0.734, ratio=0.597, max_diff=8.3%
- 0-50Hz: 45%, 50-100Hz: 26.9%, 1-3k: 4.9%
- Differenz zu Original: -45% Sub-Bass, +0% 50-100, 0% 1-3k

ZIELE:
  max_diff < 5% (von 8.3%)
  Sub-Bass 60-70% (von 71.9% zu viel)
  1000-3000Hz 4-8% (4.9% ✓ aber)
"""
import json
import sys
import numpy as np
...
def get_espeak_envelope(word, target_samples, fs):
    temp_file = "temp_espeak_fein.wav"
    try:
...
def main():
    ...
    print("FEINABSTIMMUNG PEAK1+PEAK2 (balanced_1 als Basis)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # balanced_1: sub=0.25, p2=0.30, didg=0.10, vow=0.20, saw=0.10, mh=2.50
    # Sub 71.9% → reduzieren; max_diff 8.3% → reduzieren
    tests = [
        # Reduziere Sub und p2, behalte Modulation
        (0.20, 0.25, 0.10, 0.20, 0.10, 2.50, "sub20_p2_25"),
        (0.20, 0.30, 0.10, 0.20, 0.10, 2.50, "sub20_p2_30"),
        (0.20, 0.20, 0.10, 0.20, 0.10, 2.50, "sub20_p2_20"),
        (0.15, 0.25, 0.10, 0.20, 0.10, 2.50, "sub15_p2_25"),
        (0.15, 0.30, 0.10, 0.20, 0.10, 2.50, "sub15_p2_30"),
        # Höhere Vokale + mehr MH
        (0.20, 0.25, 0.10, 0.25, 0.10, 2.50, "sub20_p2_25_vow25"),
        (0.20, 0.25, 0.10, 0.30, 0.10, 2.50, "sub20_p2_25_vow30"),
        (0.20, 0.25, 0.10, 0.30, 0.10, 3.00, "sub20_p2_25_vow30_MH3"),
        # Variation mh
        (0.20, 0.25, 0.10, 0.30, 0.10, 2.00, "sub20_p2_25_vow30_MH2"),
        (0.20, 0.25, 0.10, 0.30, 0.10, 2.50, "sub20_p2_25_vow30_MH25"),
    ]
    ...
    for sub, p2, didg, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_fein_{name}.wav"

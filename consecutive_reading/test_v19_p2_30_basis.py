"""
test_v19_p2_30_basis.py
V19 PHASE 0m — P2_30_sub25 als Basis, weitere Optimierung

BESTE BASIS: P2_30_sub25 (3/6 PASS, r=0.733, ratio=0.648)
- 0-50Hz: 49.3%, 50-100Hz: 19.2% (Summe 68.5% in 0-100Hz)
- 1000-3000Hz: 5.4% (knapp unter Ziel 4-8%)
- max_diff: 7.0%
- Centroid ratio: 0.648 (zu niedrig)

STRATEGIE: Mehr MH (4%) + Harm2 (10%) + etwas mehr Vokale (25%)
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("P2_30_SUB25 + HARM2 + MEHR MH")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # sub, harm2, didg, vow, saw, mh, p2
        (0.25, 0.05, 0.10, 0.20, 0.10, 2.50, 0.30, "P2_30_MH250"),
        (0.25, 0.10, 0.10, 0.20, 0.10, 2.50, 0.30, "P2_30_MH250_H2"),
        (0.25, 0.05, 0.10, 0.20, 0.10, 2.00, 0.30, "P2_30_MH200"),
        (0.25, 0.05, 0.10, 0.25, 0.10, 2.00, 0.30, "P2_30_MH200_v2"),
        (0.25, 0.10, 0.10, 0.20, 0.10, 2.00, 0.30, "P2_30_MH200_H2"),
        (0.25, 0.10, 0.10, 0.25, 0.10, 2.00, 0.30, "P2_30_MH200_H2_v2"),
        (0.20, 0.10, 0.10, 0.25, 0.10, 2.00, 0.30, "P2_30_MH200_sub20"),
        (0.25, 0.15, 0.10, 0.25, 0.10, 2.00, 0.25, "P2_25_MH200"),
    ]
    ...
    for sub, harm2, didg, vow, saw, mh, p2, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_p2b_{name}.wav"

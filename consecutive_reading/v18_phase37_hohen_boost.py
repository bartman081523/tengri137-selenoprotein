"""
v18_phase37_hohen_boost.py
V18 PHASE 37 — Höhen-Boost (3000-8000Hz) + Sub-Bass Feinjustierung

LEHRE AUS PHASE 36:
- T2 BÄNDER PASS (3/5 Tests) — erstes Mal!
- Sub-Bass -4.9%, 100-300Hz +4.9%, Höhen -2.5%

NEUE STRATEGIE:
- Sub-Bass reduzieren: 0.30 → 0.25
- Höhen-Rauschen erhöhen: 0.20 → 0.35 (füllt 3000-8000Hz)
- 1000-3000Hz Rauschen reduzieren: 1.20 → 1.00
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 37 — Höhen-Boost")
    print("=" * 80)
    ...
    HARMONIC_AMP = 0.15
    MID_NOISE_AMP = 0.50
    MID_HIGH_NOISE_AMP = 1.00
    CENTROID_AMP = 0.20
    HIGH_AMP = 0.40  # erhöht von 0.20
    ...
    # Höhen-Rauschen erweitert: 3000-8000Hz
    ...
    out_wav = out_dir / "synthese_v37_hohen_boost.wav"
    ...
    out_json = out_dir / "phase37_hohen_boost.json"
    output = {
        "phase": "V18 Phase 37 — Höhen-Boost",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 37: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

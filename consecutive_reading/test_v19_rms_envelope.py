"""
test_v19_rms_envelope.py
V19 PHASE 17 — Original RMS-Envelope als Modulator (statt espeak)

HYPOTHESE: Das Original hat ein spezifisches RMS-Profil pro Segment.
Wenn wir das Original-RMS als Modulator statt espeak_env verwenden,
sollte die Wellenform-Korrelation steigen.

ARCHITEKTUR:
- Pro Segment: berechne RMS-Envelope des Original-Audios in 100ms-Fenstern
- Verwende dieses Profil als Hüllkurve für die Synthese
- 1 Frame pro Segment sollte reichen
"""
import json
import sys
import numpy as np
...
def get_orig_rms_envelope(orig, start_idx, seg_samples, n_frames=100):
    """Berechne RMS-Envelope des Original-Audios in n_frames-Fenstern."""
    end_idx = min(start_idx + seg_samples, len(orig))
    seg = orig[start_idx:end_idx]
    if len(seg) < n_frames * 10:
        n_frames = max(1, len(seg) // 10)
    win = len(seg) // n_frames
    if win < 1:
        win = 1
    rms = np.zeros(n_frames)
    for k in range(n_frames):
        s = seg[k*win:(k+1)*win]
        rms[k] = np.sqrt(np.mean(s**2)) if len(s) > 0 else 0
    max_r = max(np.max(rms), 1e-12)
    rms = rms / max_r
    t_old = np.linspace(0, 1, n_frames)
    t_new = np.linspace(0, 1, seg_samples)
...
def synthese(sub, harm2, p2, vow, saw, mh, out_dir, orig, use_rms=False):
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        if use_rms:
            envelope = get_orig_rms_envelope(orig, start_idx, seg_samples, n_frames=50)
        else:
            envelope = get_espeak_envelope(seg['word'], seg_samples, SAMPLE_RATE)
        mod_strength = min(seg['mod_db'] / 10.0, 1.0)

        peak2 = np.sin(2 * np.pi * seg['peak2'] * t_seg).astype(np.float32)
        peak2_mod = peak2 * (1.0 + envelope * mod_strength * 1.0)

        harm2_seg = harmonic_2[start_idx:end_idx] * (1.0 + envelope * mod_strength * 0.5)
        ...
        vowel_mod = vowel_signal * (1.0 + envelope * mod_strength * 0.5)

        saw = sawtooth(2 * np.pi * seg['centroid'] * t_seg).astype(np.float32)
        saw_mod = saw * (1.0 + envelope * mod_strength * 0.3)

        mh_seg = mh_noise[start_idx:end_idx] * (0.3 + envelope * mod_strength * 0.7)
...
def main():
    ...
    print("ORIGINAL RMS-ENVELOPE ALS MODULATOR")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # A10_best: sub=0.20, harm2=0.20, p2=0.25, didg=0.00, vow=0.20, saw=0.25, mh=3.20
    # Teste mit Original-RMS statt espeak_env
    tests = [
        # EspeAK (baseline)
        (0.20, 0.20, 0.25, 0.20, 0.25, 3.20, False, "B1_espeak_A10"),
        # Original RMS
        (0.20, 0.20, 0.25, 0.20, 0.25, 3.20, True, "B2_rms_A10"),
        # Original RMS + K2_baseline
        (0.20, 0.20, 0.20, 0.20, 0.20, 3.10, True, "B3_rms_K2"),
        # Original RMS + p2_30
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.10, True, "B4_rms_p2_30"),
        # Original RMS + MH 340
        (0.20, 0.20, 0.25, 0.20, 0.25, 3.40, True, "B5_rms_MH340"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh, use_rms, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh, out_dir, orig, use_rms=use_rms)
        ...
        out_wav = out_dir / f"synthese_v19_t17_{name}.wav"

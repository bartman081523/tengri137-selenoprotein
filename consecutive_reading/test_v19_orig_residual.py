"""
test_v19_orig_residual.py
V19 PHASE 37 — Original-Residual (Original - Synthese) statistisch replizieren

IDEE: Berechne Original - Synthese_low (Sub-Bass + Vowel) als "was fehlt".
   Was bleibt über: Hintergrund-Atmosphäre, Echos, Verzerrungen.

Dann extrahiere die Frequenz-Verteilung dieses Residuums und repliziere es
als 1/f-verteiltes Band-gefiltertes Rauschen mit der richtigen AMPLITUDE.

Eigentlich: Berechne direkt (Original - Lowpass-Version des Originals).
Was im Hochpass bleibt, ist die "Hintergrund-Atmosphäre".
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi,
             orig, residual_mix, out_dir):
    """Mix in Original-Residual (Hochpass-Teil) statt Rauschen"""
    ...
    # Original-Hochpass-Anteil (was übrig bleibt nach 200Hz Lowpass)
    min_len = min(len(orig), n_total)
    if residual_mix > 0:
        b_lo, a_lo = butter(2, 200.0 / (SAMPLE_RATE / 2), btype='low')
        orig_low = lfilter(b_lo, a_lo, orig[:min_len]).astype(np.float32)
        orig_residual = orig[:min_len] - orig_low
        # Normalisieren
        orig_residual = orig_residual / max(np.max(np.abs(orig_residual)), 1e-12) * 0.3
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Original-Residual dazu
        if residual_mix > 0 and start_idx < min_len:
            seg_residual = orig_residual[start_idx:end_idx]
            seg_mix = seg_mix * (1.0 - residual_mix) + seg_residual * residual_mix
...
def main():
    ...
    print("ORIGINAL-RESIDUAL (Hochpass > 200Hz, statistisch)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.0, "V0_no_residual"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.05, "V1_res_005"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.10, "V2_res_010"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.15, "V3_res_015"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.20, "V4_res_020"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.30, "V5_res_030"),
        (0.20, 0.25, 0.34, 0.20, 0.20, 3.40, 1500, 3800, orig, 0.50, "V6_res_050"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, o, residual_mix, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, o, residual_mix, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_t37_{name}.wav"

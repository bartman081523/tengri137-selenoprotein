"""
v18_phase21_balance.py
V18 PHASE 21 — FORENSISCH mit BALANCE-FIX

LEHRE AUS PHASE 20:
- Forensischer Ansatz: r=0.581, max_diff=36%, 99.6% Sub-Bass
- Sub-Bass-Träger (70%) + Sägezahn-Träger dominieren
- Centroid-Layer (15%) zu leise

FIX: Balance ändern
- Sub-Bass: 0.7 → 0.5
- Peak2: 0.15 → 0.10
- Centroid-Layer: 0.15 → 0.40 (VIEL lauter!)
- Vor allem: Sägezahn-Sub-Bass raus, stattdessen Sinus-Sub-Bass mit Obertönen durch Centroid-Layer
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 21 — FORENSISCH + BALANCE-FIX")
    print("=" * 80)
    ...
    # === Balance-Konstanten (justiert) ===
    SUB_BASS_AMP = 0.5
    PEAK2_AMP = 0.10
    CENTROID_AMP = 0.50
    CENTROID_BW = 250  # breiterer Bandpass für mehr Mid-Band-Power
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Sägezahn-Träger (für Centroid-Layer)
        ...
        # Centroid-Targeting (breiterer Bandpass)
        b, a = butter_bandpass(seg['centroid'], sr, bw=CENTROID_BW)
        filtered_centroid_layer = lfilter(b, a, modulated_saw).astype(np.float32)

        # Segment-Mix (NEUE Balance)
        segment_mix = (
    ...
    out_wav = out_dir / "synthese_v21_balance.wav"
    ...
    out_json = out_dir / "phase21_balance.json"
    output = {
        "phase": "V18 Phase 21 — Forensisch + Balance",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "peak2": PEAK2_AMP, "centroid": CENTROID_AMP, "bw": CENTROID_BW},
        "tests": tests,
        "verdict": f"V18 Phase 21: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

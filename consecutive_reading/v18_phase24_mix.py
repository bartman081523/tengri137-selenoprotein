"""
v18_phase24_mix.py
V18 PHASE 24 — Sinus + Sägezahn (richtige Balance)

LEHRE AUS PHASE 23:
- Ohne Sinus: 56% Sub-Bass (zu wenig), Centroid ratio 0.628
- Mit Sinus: 88% Sub-Bass (zu viel)
- Beide brauchen wir

NEUE STRATEGIE:
- Sinus-Sub-Bass: 0.4 (für 75Hz Peak)
- Sägezahn volles Spektrum: 0.6
- Centroid-Bandpass auf Sägezahn: 0.3 (für Mitten-Anhebung)
- Rauschen 200-2000Hz: 0.2 (für 100-1000Hz-Füller)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 24 — Sinus + Sägezahn + Rauschen (richtige Balance)")
    print("=" * 80)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Rauschen mit BURUMUT-Hüllkurve
    ...
    out_wav = out_dir / "synthese_v24_mix.wav"
    ...
    out_json = out_dir / "phase24_mix.json"
    output = {
        "phase": "V18 Phase 24 — Sinus + Sägezahn + Rauschen",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "saw": SAW_AMP, "centroid": CENTROID_AMP, "noise": NOISE_AMP},
        "tests": tests,
        "verdict": f"V18 Phase 24: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

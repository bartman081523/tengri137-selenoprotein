"""
test_v19_3rd_harmonic.py
V19 PHASE 26 — 3. Harmonische (226 Hz) + höhere p2-Frequenz

H10 (5/6): sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-3500
J5 (5/6): sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-4000 (max ratio 1.139)

HYPOTHESE: 3. Harmonische (226 Hz = 3*F0) füllt 100-300Hz-Band,
   was die Korrelation mit dem Original verbessern könnte.
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, harm3, p2, vow, saw, mh, mh_lo, mh_hi, out_dir):
    """NEU: harm3 = 3. Harmonische (226Hz)."""
    ...
    harmonic_3 = np.sin(2 * np.pi * 3 * F0_PEAK1 * time_array).astype(np.float32)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        harm3_seg = harmonic_3[start_idx:end_idx] * (1.0 + espeak_env * mod_strength * 0.3)
...
def main():
    ...
    print("3. HARMONISCHE (226 Hz = 3*F0)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # H10: sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-3500
    tests = [
        # H10 baseline (ohne H3)
        (0.20, 0.25, 0.0, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "K1_no_h3"),
        # Mit H3
        (0.20, 0.25, 0.05, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "K2_h3_05"),
        (0.20, 0.25, 0.10, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "K3_h3_10"),
        (0.20, 0.25, 0.15, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "K4_h3_15"),
        (0.20, 0.25, 0.20, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "K5_h3_20"),
        # Mit H2 weniger, dafür H3
        (0.20, 0.20, 0.10, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "K6_h2_20_h3_10"),
        (0.20, 0.15, 0.15, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "K7_h2_15_h3_15"),
        # Mit H3 und mh_hi
        (0.20, 0.25, 0.10, 0.30, 0.20, 0.20, 3.50, 1500, 3800, "K8_h3_10_mh_3800"),
        (0.20, 0.25, 0.10, 0.30, 0.20, 0.20, 3.50, 1500, 4000, "K9_h3_10_mh_4000"),
        (0.20, 0.20, 0.10, 0.30, 0.20, 0.20, 3.50, 1500, 4000, "K10_h2_20_h3_10_mh_4000"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, harm3, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        synth = synthese(sub, harm2, harm3, p2, vow, saw, mh, mh_lo, mh_hi, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_t26_{name}.wav"

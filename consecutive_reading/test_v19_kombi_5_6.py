"""
test_v19_kombi_5_6.py
V19 PHASE 12 — Kombinationen für 5/6 PASS

Aktuelle Kandidaten:
- T10_sub20: r=0.776, ratio=0.860, max_diff=4.2%, 1-3k=8.5%
- T4_MH290: r=0.771, ratio=0.764, max_diff=4.9%, 1-3k=7.0%

ZIEL: r>0.7 ✓, 60-70% sub, 4-8% 1-3k, 0.85-1.15 ratio, <5% diff
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("KOMBI FÜR 5/6 (T10_sub20 + Anpassungen)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # T10_sub20: sub=0.20, harm2=0.20, p2=0.20, didg=0.00, vow=0.20, saw=0.20, mh=3.30
    # r=0.776, ratio=0.860, max_diff=4.2%, 1-3k=8.5%
    tests = [
        # T10 + weniger MH
        (0.20, 0.20, 0.20, 0.20, 0.20, 3.20, "K1_MH320"),
        (0.20, 0.20, 0.20, 0.20, 0.20, 3.10, "K2_MH310"),
        (0.20, 0.20, 0.20, 0.20, 0.20, 3.00, "K3_MH300"),
        # T10 + weniger saw
        (0.20, 0.20, 0.20, 0.20, 0.15, 3.30, "K4_saw15"),
        (0.20, 0.20, 0.20, 0.20, 0.10, 3.30, "K5_saw10"),
        # T10 + weniger H2
        (0.20, 0.15, 0.20, 0.20, 0.20, 3.30, "K6_H2_15"),
        (0.20, 0.10, 0.20, 0.20, 0.20, 3.30, "K7_H2_10"),
        # T10 + weniger vow
        (0.20, 0.20, 0.20, 0.15, 0.20, 3.30, "K8_vow15"),
        (0.20, 0.20, 0.20, 0.10, 0.20, 3.30, "K9_vow10"),
        # T10 + weniger p2
        (0.20, 0.20, 0.15, 0.20, 0.20, 3.30, "K10_p2_15"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t12_{name}.wav"

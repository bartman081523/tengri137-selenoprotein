"""
test_v19_harm3_combo.py
V19 PHASE 34 — H3 + Q7-Kombi (max_diff < 1%, ratio nahe 1.0)

BEST (max_diff < 1%):
- R4: sub=0.20, harm2=0.25, p2=0.34, mh=3.40, 1500-3800 (max_diff=0.7%, ratio=0.973)
- R5: sub=0.20, harm2=0.25, p2=0.34, mh=3.42, 1500-3800 (max_diff=0.8%, ratio=0.980)
- R7: sub=0.20, harm2=0.25, p2=0.34, mh=3.45, 1500-3700 (max_diff=0.9%, ratio=0.951)

ZIEL: H3 hinzufügen + spekulativ wave_corr
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("H3 + Q7 KOMBI (5/6 mit max_diff < 1%)")
    ...
    tests = [
        # R4 verifizieren
        (0.20, 0.25, 0.0, 0.34, 0.20, 0.20, 3.40, 1500, 3800, "S1_R4_repeat"),
        # R4 + H3
        (0.20, 0.25, 0.05, 0.34, 0.20, 0.20, 3.40, 1500, 3800, "S2_R4_h3_05"),
        (0.20, 0.25, 0.10, 0.34, 0.20, 0.20, 3.40, 1500, 3800, "S3_R4_h3_10"),
        (0.20, 0.25, 0.15, 0.34, 0.20, 0.20, 3.40, 1500, 3800, "S4_R4_h3_15"),
        # H2 weniger, H3 mehr
        (0.20, 0.20, 0.10, 0.34, 0.20, 0.20, 3.40, 1500, 3800, "S5_h2_20_h3_10"),
        (0.20, 0.20, 0.15, 0.34, 0.20, 0.20, 3.40, 1500, 3800, "S6_h2_20_h3_15"),
        # R4 + MH höher
        (0.20, 0.25, 0.0, 0.34, 0.20, 0.20, 3.45, 1500, 3800, "S7_R4_mh_345"),
        (0.20, 0.25, 0.0, 0.34, 0.20, 0.20, 3.50, 1500, 3800, "S8_R4_mh_350"),
        # Auch p2 0.33
        (0.20, 0.25, 0.0, 0.33, 0.20, 0.20, 3.40, 1500, 3800, "S9_p2_33"),
        (0.20, 0.25, 0.0, 0.35, 0.20, 0.20, 3.40, 1500, 3800, "S10_p2_35"),
    ]
    ...
    for sub, harm2, harm3, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t34_{name}.wav"

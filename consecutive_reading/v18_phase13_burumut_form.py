"""
v18_phase13_burumut_form.py
V18 PHASE 13 — BURUMUT-FORM im Spektrum + Töne-Sub-Bass

LEHRE AUS PHASE 12:
- 22 Töne → max_diff=15.9% (viel schlechter als 5 Töne)
- Mehr Töne = mehr Leck

NEUE STRATEGIE:
- Behalte 5 Töne (Sub-Bass Träger)
- Statt BURUMUT-Wörter: synthetisches BURUMUT-FORM-Spektrum
- Bandpass-gefiltertes Rauschen mit BURUMUT-Form-Spektrum (siehe V17)
- Dadurch Power in 1000-3000Hz und 300-1000Hz

Ansatz:
- 5 Sinus-Töne (50, 200, 600, 1500, 3500 + 5500) wie in v11
- + Bandpass-Rauschen mit BURUMUT-Spektrum-Profil
- = Bessere r weil mehr spektrale Details
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 13 — BURUMUT-FORM im Spektrum (Rauschen + Töne)")
    print("=" * 80)
    ...
    # === 5 Töne (wie v11) + Rauschen-Schichten ===
    ...
    # Bandpass-Rauschen-Schichten — BURUMUT-Form
    # Original hat in 1000-3000Hz 5.7% und 300-1000Hz 15.5%
    # Diese kommen aus BURUMUT-Wörtern (54% in 1000-3000Hz)
    # Wenn BURUMUT-Wörter platziert sind, sollte das schon abgedeckt sein
    # Aber lass uns mehr BURUMUT-Form dazugeben

    # Rauschen in Mid-Band (200-2000Hz) — BURUMUT-Form-Spektrum
    rausschen_mid = generiere_bandpass_rauschen(dauer_s, sr, 200, 2000, 0.05)
    out += rausschen_mid
    ...
    out_wav = out_dir / "synthese_v13.wav"
    ...
    out_json = out_dir / "phase13_burumut_form.json"
    output = {
        "phase": "V18 Phase 13 — BURUMUT-Form + Rauschen",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 13: {n_pass}/{len(tests)} PASS. r={r:.3f}, max_diff={max_diff*100:.1f}%, ratio={ratio:.3f}.",
    }

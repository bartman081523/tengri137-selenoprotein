"""
v18_phase53_original_hullkurve.py
V18 PHASE 53 — Original-Hüllkurve pro Segment für BURUMUT-Modulation

LEHRE AUS PHASE 52:
- 2/5 PASS, wave_corr immer noch < 0.01
- Espeak envelope ≠ Original envelope (Hauptursache für fehlende Wellenform-Korrelation)

ZIEL: Original-Hüllkurve pro Segment extrahieren
- Pro Segment RMS über 23.19s berechnen
- Diese Hüllkurve für Modulation verwenden
"""
import json
import sys
import numpy as np
...
def get_original_envelope(seg_idx, target_samples, sr, orig):
    """Extrahiere RMS-basierte Hüllkurve aus dem Original-Segment"""
    start = int(seg_idx * SEGMENT_DURATION * sr)
    end = int((seg_idx + 1) * SEGMENT_DURATION * sr)
    if seg_idx == 10:
        end = len(orig)
    seg = orig[start:end]
    # RMS in 100ms Fenstern
    win = int(0.1 * sr)
    n_wins = len(seg) // win
    rms = np.array([np.sqrt(np.mean(seg[i*win:(i+1)*win].astype(np.float32)**2)) for i in range(n_wins)])
    # Normalisieren auf [0.3, 1.0]
    if np.max(rms) > 0:
        rms = rms / np.max(rms) * 0.7 + 0.3
    # Auf target_samples interpolieren
...
def main():
    print("=" * 80)
    print("V18 PHASE 53 — Original-Hüllkurve pro Segment")
    print("=" * 80)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # ORIGINAL-Hüllkurve statt espeak
        orig_env = get_original_envelope(i, seg_samples, sr, orig)
    ...
    out_wav = out_dir / "synthese_v53_orig_env.wav"
    ...
    out_json = out_dir / "phase53_orig_env.json"
    output = {
        "phase": "V18 Phase 53 — Original-Hüllkurve",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "harmonic": 0.15, "mid_noise": MID_NOISE_AMP, "mid_high_noise": MID_HIGH_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP, "mid_tone": MID_TONE_AMP, "high_band": "3000-12000Hz", "envelope_source": "original_rms"},
        "tests": tests,
        "verdict": f"V18 Phase 53: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

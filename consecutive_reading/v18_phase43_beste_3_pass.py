"""
v18_phase43_beste_3_pass.py
V18 PHASE 43 — Phase 41 + Mitten-Rauschen runter + Höhen-Rauschen rauf

LEHRE AUS PHASE 41:
- 3/5 PASS, max_diff 2.7% (BEST)
- Centroid ratio 0.607

ZIEL: Centroid ratio > 0.85
- Mitten-Rauschen: 0.50 → 0.30
- Höhen: 0.20 → 0.40
- Sonst wie Phase 41
"""
import json
...
def main():
    print("=" * 80)
    print("V18 PHASE 43 — Phase 41 + Mitten runter + Höhen rauf")
    print("=" * 80)
    ...
    SUB_BASS_AMP = 0.35
    HARMONIC_AMP = 0.15
    MID_NOISE_AMP = 0.30  # reduziert
    MID_HIGH_NOISE_AMP = 1.20
    CENTROID_AMP = 0.20
    HIGH_AMP = 0.40  # erhöht
    ...
    out_wav = out_dir / "synthese_v43_beste_3_pass.wav"
    ...
    out_json = out_dir / "phase43_beste_3_pass.json"
    output = {
        "phase": "V18 Phase 43 — Beste 3/5 Basis",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 43: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

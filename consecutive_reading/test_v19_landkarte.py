"""
test_v19_landkarte.py
V19 PHASE 27 — Landkarte der 5/6-Konfigurationen

ZIEL: 10+ verschiedene 5/6-Konfigurationen identifizieren, robust gegen Variation

BEST bisher:
- H10: sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-3500
- K6:  sub=0.20, harm2=0.20, harm3=0.10, p2=0.30, mh=3.50, 1500-3500
- J5:  sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-4000
- I2:  sub=0.20, harm2=0.25, p2=0.30, mh=3.60, 1500-3500
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("LANDKARTE 5/6-KONFIGURATIONEN")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Systematische Variation um die H10-Konfiguration
    # H10: sub=0.20, harm2=0.25, p2=0.30, mh=3.50, 1500-3500
    tests = [
        # H10 verifiziert
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "L1_H10"),
        # Subtle variation
        (0.20, 0.22, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "L2_H2_22"),
        (0.20, 0.28, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "L3_H2_28"),
        (0.18, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "L4_sub_18"),
        (0.22, 0.25, 0.30, 0.20, 0.20, 3.50, 1500, 3500, "L5_sub_22"),
        # Andere p2
        (0.20, 0.25, 0.32, 0.20, 0.20, 3.50, 1500, 3500, "L6_p2_32"),
        (0.20, 0.25, 0.28, 0.20, 0.20, 3.50, 1500, 3500, "L7_p2_28"),
        # MH 3.4-3.6
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.40, 1500, 3500, "L8_mh_34"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.45, 1500, 3500, "L9_mh_345"),
        (0.20, 0.25, 0.30, 0.20, 0.20, 3.55, 1500, 3500, "L10_mh_355"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, mh_lo, mh_hi, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t27_{name}.wav"

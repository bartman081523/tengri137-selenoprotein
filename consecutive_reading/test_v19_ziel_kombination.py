"""
test_v19_ziel_kombination.py
V19 PHASE 7 — Ziel: ratio ~0.85-0.90, max_diff < 5%, 1-3k 4-8%, Sub-Bass 60-70%

GM_2: sub=0.15, harm2=0.10, p2=0.20, didg=0.10, vow=0.20, saw=0.15, mh=3.20
     → ratio=0.848, max_diff=5.0%, 1-3k=8.4%

Ziel: ratio ≥0.85, max_diff <5%, 1-3k ≤8%
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("ZIEL-KOMBINATION: ratio 0.85-1.15, max_diff <5%, 1-3k 4-8%")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # GM_2: sub=0.15, harm2=0.10, p2=0.20, didg=0.10, vow=0.20, saw=0.15, mh=3.20
        # Erhöhe saw → mehr high freq, höhere ratio
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 3.20, "ZK1_saw20"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.25, 3.20, "ZK2_saw25"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 3.10, "ZK3_MH310"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 3.00, "ZK4_MH300"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 2.90, "ZK5_MH290"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.20, "ZK6_vow25"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.10, "ZK7_vow25_MH310"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.00, "ZK8_vow25_MH300"),
        # Try balanced H2
        (0.15, 0.15, 0.20, 0.10, 0.20, 0.20, 3.20, "ZK9_H2_15"),
        (0.15, 0.10, 0.20, 0.10, 0.20, 0.20, 3.15, "ZK10_MH315"),
    ]
    ...
    for sub, harm2, p2, didg, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_zk_{name}.wav"

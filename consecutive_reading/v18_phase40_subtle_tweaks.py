"""
v18_phase40_subtle_tweaks.py
V18 PHASE 40 — Sub-Tweaks auf Phase 36 (3/5 PASS)

LEHRE AUS PHASE 36:
- max_diff 4.9% (T2 PASS)
- Centroid ratio 0.696
- Sub-Bass -4.9%, 100-300Hz +4.9%

ZIEL: Sub-Bass leicht erhöhen, 100-300Hz leicht reduzieren (für T4)
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 40 — Sub-Tweaks auf Phase 36")
    print("=" * 80)
    ...
    # Phase 36 + Sub-Bass +0.10, Mitten-Rauschen -0.10
    SUB_BASS_AMP = 0.40
    HARMONIC_AMP = 0.20
    MID_NOISE_AMP = 0.40
    ...
    out_wav = out_dir / "synthese_v40_subtle.wav"
    ...
    out_json = out_dir / "phase40_subtle.json"
    output = {
        "phase": "V18 Phase 40 — Sub-Tweaks",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "tests": tests,
        "verdict": f"V18 Phase 40: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }

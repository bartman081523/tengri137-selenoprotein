"""
test_v19_ohne_didg.py
V19 PHASE 10 — T6_didg00 als Basis: r=0.776, ohne Didgeridoo

HYPOTHESE: Didgeridoo-Träger erzeugt Rauschen im 100-1000Hz-Band
und senkt die Korrelation. Ohne Didgeridoo → höhere r, niedrigere 1-3k.

T6_didg00: sub=0.15, harm2=0.15, p2=0.20, didg=0.00, vow=0.20, saw=0.20, mh=3.30
  → r=0.776, ratio=0.885, max_diff=5.1%, 1-3k=8.9%

ZIEL: max_diff < 5%, 1-3k ≤ 8% (bei Erhalt von r > 0.7)
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("OHNE DIDGERIDOO (T6_didg00: r=0.776 als Basis)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # T6: sub=0.15, harm2=0.15, p2=0.20, didg=0.00, vow=0.20, saw=0.20, mh=3.30
    tests = [
        # Original
        (0.15, 0.15, 0.20, 0.20, 0.20, 3.30, "T6_baseline"),
        # Weniger saw
        (0.15, 0.15, 0.20, 0.20, 0.15, 3.30, "T6_saw15"),
        (0.15, 0.15, 0.20, 0.20, 0.10, 3.30, "T6_saw10"),
        (0.15, 0.15, 0.20, 0.20, 0.05, 3.30, "T6_saw05"),
        # Weniger MH
        (0.15, 0.15, 0.20, 0.20, 0.20, 3.10, "T6_MH310"),
        (0.15, 0.15, 0.20, 0.20, 0.20, 3.00, "T6_MH300"),
        (0.15, 0.15, 0.20, 0.20, 0.20, 2.90, "T6_MH290"),
        (0.15, 0.15, 0.20, 0.20, 0.20, 2.80, "T6_MH280"),
        # Weniger H2
        (0.15, 0.10, 0.20, 0.20, 0.20, 3.30, "T6_H2_10"),
        (0.15, 0.20, 0.20, 0.20, 0.20, 3.30, "T6_H2_20"),
    ]
    ...
    for sub, harm2, p2, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_t10_{name}.wav"

"""
test_v19_peak1_plus_peak2.py
V19 PHASE 0n — Peak1 + Peak2 mit SHARED espeak envelope

KRITISCH: Original hat 12.96% in 0-50Hz + 50.58% in 50-100Hz.
Synthesen haben 48% in 0-50Hz + 19% in 50-100Hz.

HYPOTHESE: Original-Sub-Bass ist Peak1 (75Hz) + Peak2 (86Hz) BEIDE mit espeak_env
          moduliert — beide Träger schwingen MIT der BURUMUT-Envelope.
NICHT-HYPOTHESE: Peak1 kontinuierlich, Peak2 nur als Effekt.

NEUE ARCHITEKTUR:
  1. Peak1 (75Hz) - kontinuierlicher Träger (75%)
  2. Peak2 (per Segment 53-86Hz) - MIT espeak_env (40%)
  3. Sub-Bass = Peak1 + Peak2-2-Harm (43-172Hz)
"""
import json
import sys
import numpy as np
...
def get_espeak_envelope(word, target_samples, fs):
    temp_file = "temp_espeak_p1p2.wav"
    try:
...
def synthese(sub, p2, didg, vow, saw, mh, out_dir):
    """
    sub = Peak1 (75Hz) kontinuierlich
    p2 = Peak2 (53-86Hz) mit espeak_env-Modulation
    """
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        # Peak2: per Segment, mit espeak_env-Modulation
        peak2 = np.sin(2 * np.pi * seg['peak2'] * t_seg).astype(np.float32)
        # Variante 1: mit espeak_env multiplizieren (pulsiert)
...
def main():
    ...
    print("PEAK1+PEAK2 BEIDE MIT espeak_env (Sub-Bass-Verteilung 50-100Hz)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-50':<8} {'50-100':<8} {'1-3k':<8}")
    print("-" * 100)

    tests = [
        # sub, p2, didg, vow, saw, mh
        (0.25, 0.30, 0.10, 0.20, 0.10, 2.50, "balanced_1"),
        (0.30, 0.30, 0.10, 0.20, 0.10, 2.50, "balanced_2"),
        (0.20, 0.40, 0.10, 0.20, 0.10, 2.50, "balanced_3"),
        (0.25, 0.40, 0.10, 0.20, 0.10, 2.50, "balanced_4"),
        (0.30, 0.40, 0.10, 0.20, 0.10, 2.50, "balanced_5"),
        (0.20, 0.50, 0.10, 0.20, 0.10, 2.50, "balanced_6"),
        (0.25, 0.50, 0.10, 0.20, 0.10, 2.50, "balanced_7"),
        (0.30, 0.50, 0.10, 0.20, 0.10, 2.50, "balanced_8"),
    ]

    best_n = 0
    best = ""
    for sub, p2, didg, vow, saw, mh, name in tests:
        synth = synthese(sub, p2, didg, vow, saw, mh, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_p1p2_{name}.wav"

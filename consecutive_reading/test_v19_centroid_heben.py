"""
test_v19_centroid_heben.py
V19 PHASE 0i — Centroid anheben durch mehr MH + weniger Sub

BESTE BASIS: H3_mh250 (Centroid 0.685, knapp unter 0.85)
PROBLEM: Centroid zu niedrig weil Sub-Bass bei 66% dominiert

STRATEGIE: Sub-Bass durch 2. Harmonische ergänzen (150.7Hz)
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, didg, vow, saw, mh, p2, out_dir):
    ...
    # 2. Harmonische 150.7Hz (in 100-300Hz-Band)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        seg_mix = (
            ...
            harmonic_2[start_idx:end_idx] * harm2 +
...
def main():
    ...
    print("MIT 2. HARMONISCHER (Centroid heben)")
    print(f"{'Name':<40} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'100-300':<10} {'1-3k':<8}")
    print("-" * 110)

    tests = [
        # sub, harm2, didg, vow, saw, mh, p2
        (0.25, 0.10, 0.10, 0.15, 0.10, 2.50, 0.05, "harm2_10"),
        (0.25, 0.15, 0.10, 0.15, 0.10, 2.50, 0.05, "harm2_15"),
        (0.25, 0.20, 0.10, 0.15, 0.10, 2.50, 0.05, "harm2_20"),
        (0.25, 0.25, 0.10, 0.15, 0.10, 2.50, 0.05, "harm2_25"),
        (0.20, 0.20, 0.10, 0.15, 0.10, 2.50, 0.05, "sub20_harm20"),
        (0.20, 0.25, 0.10, 0.15, 0.10, 2.50, 0.05, "sub20_harm25"),
        (0.20, 0.30, 0.10, 0.15, 0.10, 2.50, 0.05, "sub20_harm30"),
        (0.15, 0.30, 0.10, 0.15, 0.10, 2.50, 0.05, "sub15_harm30"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, didg, vow, saw, mh, p2, name in tests:
        synth = synthese(sub, harm2, didg, vow, saw, mh, p2, out_dir)
        ...
        print(f"{name:<40} {r:.3f}   {wave_corr:+.4f}   {ratio:.3f}   {max_diff*100:5.1f}%   "
              f"{sb['0-100Hz']*100:5.1f}%   {sb['100-300Hz']*100:5.1f}%   {sb['1000-3000Hz']*100:5.1f}%  [{n}/6]")

        out_wav = out_dir / f"synthese_v19_ch_{name}.wav"

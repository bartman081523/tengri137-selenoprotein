"""
test_v19_3band_mh.py
V19 PHASE 19 — 3-Band Mid-High-Architektur

PROBLEM: 1-3k > 8% UND ratio < 0.85 schließen sich aus.
LÖSUNGS-IDEAL: Statt einem MH-Bandpass (1-3k), verwende zwei
separate Bänder:
   MH1: 1000-2000 Hz (trägt mehr zur ratio bei, weniger 1-3k)
   MH2: 3000-8000 Hz (oberhalb 1-3k, füllt 3-8k-Band)

EFFEKT: 1-3k sinkt, 3-8k steigt → ratio steigt ohne 1-3k > 8%
"""
import json
import sys
import numpy as np
...
def synthese(sub, harm2, p2, vow, saw, mh_low, mh_high, mh1_weight, mh2_weight, out_dir):
    """3-Band MH: 1000-2000 + 2000-8000 (statt 1000-3000)."""
    ...
    # MH1: 1000-2000 Hz (vermindert 1-3k-Anteil)
    b_mh1, a_mh1 = butter_bandpass_range(1000, 2000, SAMPLE_RATE)
    mh1_noise = lfilter(b_mh1, a_mh1, white).astype(np.float32)
    # MH2: 2000-8000 Hz (hebt 3-8k-Anteil → ratio)
    b_mh2, a_mh2 = butter_bandpass_range(2000, 8000, SAMPLE_RATE)
    mh2_noise = lfilter(b_mh2, a_mh2, white).astype(np.float32)
    ...
    for i, seg in enumerate(SEGMENTS_DATA):
        ...
        mh1_seg = mh1_noise[start_idx:end_idx] * (0.3 + espeak_env * mod_strength * 0.7)
        mh2_seg = mh2_noise[start_idx:end_idx] * (0.3 + espeak_env * mod_strength * 0.7)
        ...
        seg_mix = (
            ...
            vowel_mod * vow +
            saw_mod * saw +
            mh1_seg * mh_low * mh1_weight +
            mh2_seg * mh_high * mh2_weight
        ).astype(np.float32)
...
def main():
    ...
    print("3-BAND MH (1000-2000 + 2000-8000)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Original-Architektur: 1-3k ≤ 8% + ratio ≥ 0.85 schließen sich aus
    # 3-Band: MH1 (1000-2000) und MH2 (2000-8000) separat
    tests = [
        # MH1=1000-2000, MH2=2000-8000
        # sub=0.20, harm2=0.20, p2=0.30, vow=0.20, saw=0.20
        # mh_low, mh_high, mh1_weight, mh2_weight
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.0, 3.0, 1.0, 1.0, "D1_baseline_3band"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.0, 3.5, 1.0, 1.0, "D2_mh2_35"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 3.0, 4.0, 1.0, 1.0, "D3_mh2_40"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 2.5, 3.0, 1.0, 1.0, "D4_mh1_25"),
        # Mehr MH2, weniger MH1
        (0.20, 0.20, 0.30, 0.20, 0.20, 2.5, 3.5, 0.5, 1.5, "D5_mh1_25_mh2_35"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 2.0, 3.5, 0.5, 1.5, "D6_mh1_20_mh2_35"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 2.0, 4.0, 0.5, 1.5, "D7_mh1_20_mh2_40"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 2.0, 4.0, 0.3, 2.0, "D8_mh1_20_mh2_40_w"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 1.5, 4.0, 0.3, 2.0, "D9_mh1_15_mh2_40_w"),
        (0.20, 0.20, 0.30, 0.20, 0.20, 1.0, 4.0, 0.3, 2.0, "D10_mh1_10_mh2_40_w"),
    ]

    best_n = 0
    best = ""
    for sub, harm2, p2, vow, saw, mh_low, mh_high, w1, w2, name in tests:
        synth = synthese(sub, harm2, p2, vow, saw, mh_low, mh_high, w1, w2, out_dir)
        ...
        out_wav = out_dir / f"synthese_v19_t19_{name}.wav"

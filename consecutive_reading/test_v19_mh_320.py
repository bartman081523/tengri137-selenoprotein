"""
test_v19_mh_320.py
V19 PHASE 4 — MH-Layer Feinabstimmung (3.00-3.50)

HYPOTHESE: Mit MH=3.20 sind ratio + 1-3kHz beide im Ziel
3sb_baseline: MH=2.50 → ratio=0.631, 1-3k=5.2%
MH35: MH=3.50 → ratio=0.943, 1-3k=9.5%
"""
import json
import sys
import numpy as np
...
def main():
    ...
    print("MH-FEINABSTIMMUNG (3.00 - 3.50)")
    print(f"{'Name':<35} {'r':<7} {'wave':<10} {'ratio':<7} {'diff':<8} {'0-100':<8} {'1-3k':<8}")
    print("-" * 100)

    # Basis: 3sb_vow25_MH3 (r=0.741, ratio=0.754, max_diff=4.9%)
    tests = [
        # sub, harm2, p2, didg, vow, saw, mh
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.10, "MH310"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.20, "MH320"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.30, "MH330"),
        (0.15, 0.10, 0.20, 0.10, 0.25, 0.20, 3.40, "MH340"),
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.20, 3.10, "MH310_vow30"),
        (0.15, 0.10, 0.20, 0.10, 0.30, 0.20, 3.20, "MH320_vow30"),
        (0.20, 0.10, 0.20, 0.10, 0.25, 0.20, 3.20, "MH320_sub20"),
        (0.20, 0.10, 0.20, 0.10, 0.25, 0.20, 3.10, "MH310_sub20"),
        (0.10, 0.10, 0.20, 0.10, 0.25, 0.20, 3.20, "MH320_sub10"),
        (0.15, 0.05, 0.20, 0.10, 0.25, 0.20, 3.20, "MH320_H2_05"),
    ]
    ...
    for sub, harm2, p2, didg, vow, saw, mh, name in tests:
        ...
        out_wav = out_dir / f"synthese_v19_320_{name}.wav"

"""
v18_phase49_sawtooth_sub.py
V18 PHASE 49 — Sub-Bass von sine zu sawtooth wechseln

LEHRE AUS PHASE 48:
- 3/5 PASS, T1+T2+T3
- Sub-Bass 0-100Hz: 63.7% (Orig 63.5%, +0.2%)
- Centroid ratio 0.818 (ZU NIEDRIG)

ZIEL: Sub-Bass breitbandig machen (sawtooth mit Harmonischen)
- Phase 48 + Sub-Bass sawtooth statt sine
"""
import json
import sys
import numpy as np
...
def main():
    print("=" * 80)
    print("V18 PHASE 49 — Sawtooth Sub-Bass")
    print("=" * 80)
    ...
    SUB_BASS_AMP = 0.30  # leicht reduziert wegen Harmonischen
    HARMONIC_AMP = 0.15
    MID_NOISE_AMP = 0.45
    ...
    # SAWTOOTH statt sine
    sub_bass_carrier = sawtooth(2 * np.pi * F0_PEAK1 * time_array).astype(np.float32)
    ...
    out_wav = out_dir / "synthese_v49_saw_sub.wav"
    ...
    out_json = out_dir / "phase49_saw_sub.json"
    output = {
        "phase": "V18 Phase 49 — Sawtooth Sub-Bass",
        "n_pass": n_pass,
        "n_tests": len(tests),
        "spektrum_r": float(r),
        ...
        "balance": {"sub": SUB_BASS_AMP, "harmonic": HARMONIC_AMP, "mid_noise": MID_NOISE_AMP, "mid_high_noise": MID_HIGH_NOISE_AMP, "centroid": CENTROID_AMP, "high": HIGH_AMP, "mid_tone": MID_TONE_AMP, "high_band": "3000-12000Hz", "sub_waveform": "sawtooth"},
        "tests": tests,
        "verdict": f"V18 Phase 49: {n_pass}/{len(tests)} PASS. r={r:.3f}, wave_corr={wave_corr:.3f}, ratio={ratio:.3f}.",
    }
